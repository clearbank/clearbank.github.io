...
To submit a payment, use the [POST /v1/payments/sepa-instant](#send-a-sepa-instant-payment) endpoint. You will receive the [SEPA Instant Payment Settled webhook](#sepa-instant-payment-settled-webhook) and the [Customer Accounts Transaction Completed webhook](#customer-accounts-transaction-completed-webhook).

![A message flow diagram explaining how to send a SEPA ICT payment](/assets/images/send-a-sepa-ict-payment.svg)

<Callout colour="blue">You must integrate with the customer accounts transaction settled webhook to use this product. When you have a payment that lands in your suspense account, you won't know where it's originated unless you've integrated with the customer accounts transaction completed webhook.
</Callout>
...
Recall requests should be responded to within 15 business days. ClearBank will send you reminders to respond via Freshdesk. If after 15 business days you do not respond to the recall, ClearBank will automatically reject the request.
