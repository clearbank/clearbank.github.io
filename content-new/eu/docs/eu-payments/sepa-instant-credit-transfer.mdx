...
You can send, receive, and recall SEPA ICT payments. You can also receive incoming recall requests that you can accept or reject. If you do not respond to incoming recall requests, they will be automatically rejected.

## Sending a SEPA ICT payment

To submit a payment, use the [POST /v1/payments/sepa-instant](#send-a-sepa-instant-payment) endpoint. You will receive the [SEPA ICT Payment Settled webhook](#sepa-instant-payment-settled-webhook) and the [Customer Accounts Transaction Settled webhook](#customer-accounts-transaction-completed-webhook).

![A message flow diagram explaining how to send a SEPA ICT payment](/assets/images/send-a-sepa-ct-payment.svg)
...
You can both send and receive recalls. Recall requests remain valid for a maximimum of 15 business days.
...
You may receive an inbound recall request via the [SEPA Instant Recall Request Received webhook](#sepa-instant-recall-request-received-webhook). To respond to the recall request, use the [/v1/payments/sepa-instant/{paymentId}/recall-response](#respond-to-a-sepa-instant-recall-request) endpoint.
