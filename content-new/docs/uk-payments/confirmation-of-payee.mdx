...
- Use the [Get CSR](#get-certificate-type) endpoint to get the Certificate Signing Request (CSR) .csr file. The softwareStatementID and certificateType inputs are required inputs.
- Use the [Create a new certificate](#create-a-new-certificate) endpoint to upload an X.509 certificate in a PEM format for use in OpenBanking applications. The certificate can be associated with the Software Statement ID to authenticate and enable secure communication with Open Banking APIs. This POST endpoint provides a secure and straightforward approach to obtain a certificate in a PEM format.
