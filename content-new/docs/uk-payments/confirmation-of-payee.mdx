...
- Use the [Get certificate type](#get-certificate-type) endpoint to get the Certificate Signing Request type. The CSR type is used to identify the algorithm used for encryption along with the key size for the certificate generation process. The softwareStatementID and certificateType inputs are required inputs. This GET endpoint allows the ability to retrieve the CSR certificate type with their application's software statement ID. This ensures all parties are compliant with Open Banking APIs so that a secure method is used to get the relevant information.
- Use the [Create a new certificate](#create-a-new-certificate) endpoint to generate an X.509 certificate in a PEM format for use in OpenBanking applications. The certificate can be associated with the Software Statement ID to authenticate and enable secure communication with Open Banking APIs. This POST endpoint provides a secure and straightforward approach to obtain a certificate in a PEM format.
