...
- If a cheque cannot be submitted to the ICS scheme, for example, due to low image quality or missing signature, you will receive a 400 response when you submit the cheque.
- If or the cheque fails at the ICS scheme, you will receive the transaction failed report from ClearBank on Freshdesk.
- If a decision is made by the issuing bank to not pay the cheque, you will receive the [Cheque Not Paid](#cheque-not-paid-webhook) webhook.

#### Image requirements
For cheque images to successfully validate, they must satisfy the following requirements:

| File type | Encoding | Resolution | Front image size | Back image size| 
|--|--|--|--|--|
| Bitonal JPEG | Base64 encoded string | 200DPI | 600 to 800,000 byes | 58 to 90,000 bytes |
| Bitonal TIFF | Base64 encoded string | 200DPI | 600 to 90,000 byes | 58 to 90,000 bytes |

Cheque images must also be clear, uncrumpled, have visible edges, and without folded corners.
