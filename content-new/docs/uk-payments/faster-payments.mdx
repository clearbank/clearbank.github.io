...
When sending payments through our API, Faster Payments are initiated via the POST /{vn}/Payments/FPS endpoint, and confirmation of settlement is provided to you via the [Transaction Settled webhook](#transaction-settled-webhook).
...
**Faster Payments abusive messages validation**

To protect payment recipients from harm and abuse, outbound payments are subject to screening against a profanity list maintained by Faster Payments scheme. The profanity list can be found on the Reference Documents page of the ClearBank Knowledge Centre which can be accessed through the ClearBank Portal. Inbound payments are subject to screening by the remitting bank and Faster Payments scheme.

The screening specifically looks at the Reference string of the RemittanceInformation object. If a reference is removed from this field for a hit against the profanity list, the field will be blank when the payment reaches the beneficiary.

The outcome of profanity screening can be identified in the SupplementaryData field of the Transaction Settled webhook as follows:

| Abusive message screening outcome    | SupplementaryData value for structured reference   | SupplementaryData value for unstructured reference  |
|--------------------------------------|----------------------------------------------------|-----------------------------------------------------|
| Pass                                 | `AbusiveMesageScreeningOutcome:Pass`                 | N/A                                                 |
| Fail                                 | `AbusiveMesageScreeningOutcome:Fail`                 | `AbusiveMesageScreeningOutcome:Fail` `AbusiveMesageScreeningIdentifiers:` `paymentInstruction.creditTransfer.remittanceInformation.unstructured.additionalReferenceInformation.reference` |                         
