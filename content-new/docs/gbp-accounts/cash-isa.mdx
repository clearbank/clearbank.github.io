...
If the cash ISA is under 14 days old and has had money deposited, use the [cancel a cash ISA](#cancel-a-cash-ISA) endpoint instead.
...
Cash ISAs can be cancelled within 14 days of account creation. The endpoint also closes the account.

The cancellation period is based on the acceptance date field. For example, an account with created on the 1st of the month can be cancelled up to 23:59 on the 14th day of the month.
...
During transfer out, you move funds to a transfer out account created during onboarding. If the transfer out is successful, move the funds from the transfer out account to the new provider's account. If the transfer out fails, return the funds from the transfer out account back to the ClearBank account.

There is a mandatory 15 day waiting period. **If you requested an account closure on the 1st of the month, the waiting period ends at 00:00 on the 16th.** Once the transfer is successful and the waiting period has completed, request to close the account using the [POST /v1/accounts/{accountId}/closure](#close-a-cash-isa) endpoint.
...
3. Wait at least the remainder of the 15 working day waiting period from account closure to ensure the transfer has been successful.
