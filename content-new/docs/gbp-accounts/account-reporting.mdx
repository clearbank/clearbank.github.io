...
<Callout colour="blue">
Note that you will observe an expected delay in receiving the Statement Ready For Download webhook. The time taken to generate a statement varies depending on the number of settled transactions included in the request.
If the endDateTimeUtc exceeds T00:00:00Z (includes the next calendar day), the statement will not be available until reconciliation has been completed for both days.
