...
If the transfer out fails, the new provider will provide you a reason the transfer failed and will return funds to the transfer out hub. Use the [POST /v1/isas/{accountId}/transfer-outs/rejection](#transfer-out-rejected) to let ClearBank know the transfer out was unsuccessful and to unfreeze the account.
