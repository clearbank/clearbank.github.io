...
During the transfer out process, you move funds to a transfer hub account created during onboarding. The cash ISA is then frozen. If the transfer out is successful, you can then move the funds from the transfer hub to the new provider's account. There is a mandatory 15 day waiting period before you can close the account. If you requested a transfer out on the 1st of the month, the waiting period ends at 00:00 on the 16th. After that, you can close the account.

If the transfer out fails, the new provider will provide you a reason the transfer failed and they will return funds to the transfer hub. Use the [POST /v1/isas/{accountId}/transfer-outs/rejection](#transfer-out-rejected) endpoint to let ClearBank know the transfer out was unsuccessful and to unfreeze the account and return the account to the state it was before you requested the transfer out. Once the account is unfrozen, transfer the funds back into the cash ISA. The cash ISA will now continue to exist.
...
2. Once the account has been credited the final interest payment, withdraw the full balance of the account to the transfer hub account created during onboarding. From there, you can send the funds to the new provider.
...
ClearBank supports full or partial transfers in of cash ISAs. You must have already created the cash ISA before beginning a transfer in.

During the transfer in process, you transfer the cash ISA funds to a transfer hub account created during onboarding. Once the transfer in is approved, you can then move the funds from the hub account into the cash ISA.

To transfer in a cash ISA:
1. Use the [POST /v1/isas/{accountId}/transfer-in](#transfer-in-a-cash-isa) endpoint.
> You must provide a unique payment reference in the `PaymentReference` field when calling this endpoint. This reference will be used for reconciliation when the funds enter the transfer hub.
2. The other provider should provide you with the funds and the transfer history form.
3. Use the [PUT /v1/isas/{accountId}/transfer-in/{transferInId}/transfer-history-form](#submit-a-transfer-history-form) to submit the transfer history form to ClearBank.
> OPTIONAL To the status of a transfer in, use [GET /v1/isas/{accountId}/transfer-in/{transferInId}](#get-information-on-a-transfer-in)
4. Reconcile the funds using the hub account you created during onboarding.
5. Once the transfer in has been approved, move the funds from the transfer hub to the ISA.
...
## Setting customer eligibility

When you update a customer's profile using the [FSCS protected deposits endpoints](../embedded-banking/fscs-protected-deposits), if their [ISA eligibility](https://www.gov.uk/individual-savings-accounts/if-you-move-abroad) changes, ClearBank will automatically update the customer's profile and set them as ineligible.

<Callout colour="blue">
<p>A customer is ineligible if their date of birth or national insurance number are invalid, or if their country of residence is not GBR. They will not be able to make any further subscriptions to the ISA until they update their date of birth or NI number or confirm they are are a UK resident.</p>

<p>If a customer is a UK resident, but has an address outside the UK, they will be able to subscribe for the remainder of the tax year, but will have to update their address and redeclare to subscribe in the following tax year.</p>
</Callout>

If the customer becomes eligible again and wants to resubscribe to an ISA, you should:
1. Update their profile with that information using the [customer profile endpoints](../embedded-banking/fscs-protected-deposits)
2. Resubscribe to an ISA using the [POST /v1/isas/{accountId}/declaration endpoint](#set-a-clients-eligiblity)

<EndpointBlock
  type="post"
  title="Set a client's eligibility"
  endpoints={[
    {
