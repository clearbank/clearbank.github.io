...
ClearBank supports full or partial transfers in of cash ISAs. The cash ISA must have already been opened using the endpoints above.

To transfer in a cash ISA, use the [POST /v1/isas/{accountId}/transfer-in](#transfer-in-a-cash-isa) endpoint. This begins the transfer in process. Use the [PUT /v1/isas/{accountId}/transfer-in/{transferInId}/transfer-history-form](#submit-a-transfer-history-form) to submit the transfer history form.

To check the status of a transfer in, use [GET /v1/isas/{accountId}/transfer-in/{transferInId}](#get-information-on-a-transfer-in).
...
When you update a customer's profile using the [FSCS protected deposits endpoints](../embedded-banking/fscs-protected-deposits), if their [ISA eligibility](https://www.gov.uk/individual-savings-accounts/if-you-move-abroad) changes, ClearBank will automatically update the customer's profile and unsubscribe them from their ISA.

<Callout colour="blue">
Unsubscription means a customer cannot deposit money into their ISA. They can still withdraw money and recieve UK tax relief on funds held in it.
</Callout>

If the customer becomes eligible again and wants to resubscribe, you should:
