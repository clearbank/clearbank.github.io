...
As cash ISAs have a maximum deposit limit per tax year, if you are trying to deposit funds that go over that limit, you will receive the [Transaction Rejected webhook](#transaction-rejected-webhook).
...
> You must provide a unique payment reference in the `PaymentReference` field when calling this endpoint. This reference will be used for reconciliation when the funds leave the transfer hub.
3. The previous manager of the cash ISA will move funds into the transfer hub and send you the transfer history form.
...
5. Once the transfer in has been approved, move the funds from the transfer hub to the cash ISA and reconcile them (identified by the `PaymentReference` field).
