...
Due to the 5 second timeout we recommend doing as little processing as possible before responding the webhook. We would recommend using an internal queuing solution so that the processing of the webhook can be handled asynchronously which means you are still able to respond synchronously.
