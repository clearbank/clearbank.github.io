...
Webhooks are asynchronous events and can be delivered in any order. We provide at-least-once delivery of webhooks. Because webhooks may be sent more than once, your webhook handlers must check for idempotency based on the payload of the webhook.
...
The ranges of IP addresses that ClearBank webhooks are sent from are different for the simulation and production environments. On 24 August the IP address range is increasing. All the IP addresses listed here should be included on your allow list, so that webhooks are not blocked by your security. 
...
From 24 August 2023 the range of public IP addresses to support your interaction with ClearBank webhooks is:
- Simulation Public IP address range: `51.145.122.16/28`, `20.39.213.0/28`, `172.187.147.144/28`, `20.117.212.176/28`
- Production Public IP address range: `51.145.122.32/28`, `172.187.179.192/28`, `172.187.243.32/28`, `20.117.189.64/28`
...
We have provided an example C# controller in the [ClearBank fi-api-101 Github Repository](https://github.com/clearbank/fi-api-101) which shows how to calculate the hash value and verify the digital signature. Once you know the webhook can be trusted, you can respond.
...
ClearBank will retry failed webhooks every 15 minutes for 24 hours or until you return a successful response.
...
Due to the 5 second timeout window that applies to successful webhook responses, it makes sense to minimise the processing that is done before responding to the webhook. We recommend verifying and responding to the webhook synchronously, then using an internal queuing solution to handle any further processing asynchronously (including the check for idempotency).
