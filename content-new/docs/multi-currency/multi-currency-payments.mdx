...
If you would prefer not to have the rescheduled payment processed, you can cancel the payment by using the cancellation endpoints. The cancellation must be received more than 1 hour before the scheduled processing time. Note that to be able to cancel a payment, you must have specified a **batch ID** in the original payment request.
