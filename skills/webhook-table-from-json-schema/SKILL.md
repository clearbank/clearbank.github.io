...
- For `type: object`, set `Format`, `Min length`, `Max length`, and `Pattern` to `-`, then reference the object sub-table in `Description`.
...
Rules:
- Map JSON Schema `minimum`/`maximum` (and exclusive variants) into concise `Constraints` text.
...
- Map from `minLength` / `maxLength` for string fields.
- Otherwise `-`.
...
- For object fields, append `See \`ObjectName\`.`
...
3. String length and regex constraints are transferred accurately.
4. Enum values are documented in `Pattern` column when no regex is provided.
5. All `See \`ObjectName\`.` references point to an existing sub-table section.
