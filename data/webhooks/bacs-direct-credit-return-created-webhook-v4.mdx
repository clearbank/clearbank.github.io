...
| `TransactionCode`             |*Required*  Transaction code for the Bacs Direct Credit transaction being returned. It is always going to be RA. The Transaction code RA means that funds associated with an unapplied credit are being returned using an ARUCS payment instruction through Bacs.<br /> Type: `string`<br/>Max. Length: `2`<br/>Min. Length: `2`<br /> Pattern: `RA` |
