...
**This webhook notifies that the settlement of an FX trade has failed. In contrast to the [FX Trade Cancelled](fx-trade-cancelled-v1) webhook, this applies to transactions that are not settled on the same day as booked. In practise, the settlement only fails if the payer account is insufficiently provisioned at the moment of settlement.**
