...
![A customer deposits £2,000 into ISA1, and holds £1,500 in ISA2. The financial institution(s) responsible for ISA2 will receive this webhook](/assets/images/diagrams/uk/cash-isa/cash-isa-subscription-updated.svg)
...
| ----- | ---- | ------ | -------- | -------------- | ------- | ----------- |
| `AccountId` | string | uuid | Yes | - | - | The unique identifier for this ISA. |
| `TaxYear` | string | - | Yes | 5-5 | `^[0-9]{2}/[0-9]{2}$` | The current tax year. |
| `CashIsaStatus` | string | - | Yes | - | `None`, `Inactive`, `Active`, `Cancelled`, `Closed`, `TransferredOut` | The status of this ISA. |
| `NetSubscription` | decimal | - | Yes | - | - | The net amount deposited to and withdrawn from this ISA in the current tax year. This value can be negative if withdrawals exceed deposits. |
| `RemainingSubscriptionAllowance` | decimal | - | Yes | - | - | The remaining amount the customer can deposit into this cash ISA this tax year, which reflects the subscriptions this customer has made across all ISAs managed by ClearBank. |
| `AnnualAllowanceUsed` | decimal | - | Yes | - | - | The maximum amount that has been deposited into the cash ISA this tax year. |
| `RemainingSubscriptionAllowanceUpdatedDateTime` | string | date-time | Yes | - | - | The UTC date and time when this allowance was updated. |
