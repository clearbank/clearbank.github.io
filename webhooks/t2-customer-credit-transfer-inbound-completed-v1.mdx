...
|`CreditorAccount`         |*Optional*<br />Information about the counterpart in a given transaction. <br/>Type: `object`. Contains the `Iban`, `OtherAccountId` as a oneOf - You will either receive Iban or OtherAccountId.<br/><br/>`Iban`: International Bank Account Number associated with the party.<br/>Type: `string`<br/>Pattern:`[A-Z]{2,2}[0-9]{2,2}[a-zA-Z0-9]{1,30}` <br/> <br/> `OtherAccountId`: Normally an alternative account identifier when IBAN is not available. Sometimes, the remitter provides an IBAN in the OtherAccountId field. In such cases, funds are credited to the IBAN. <br/> Type: `string` <br/> Pattern: `[0-9a-zA-Z/\-\?:\(\)\.,'\+]\|([0-9a-zA-Z/\-\?:\(\)\.,'\+]\|[0-9a-zA-Z/\-\?:\(\)\.,'\+ ]{1,32}[0-9a-zA-Z/\-\?:\(\)\.,'\+])`|
