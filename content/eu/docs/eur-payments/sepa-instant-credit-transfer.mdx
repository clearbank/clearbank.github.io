...
You can:
- [Send a SCT Inst payment](#send-a-sepa-instant-payment)
- [Receive SCT Inst payment](#receive-a-sepa-instant-payment)
- [Recall a SCT Inst payment](#send-a-recall-request) (integrating with recall payments is required)
- [Receive and respond to a recall request](#receive-and-respond-to-a-recall-request)
- [Simulate a SCT Inst inbound payment and recall request](#simulate-a-sepa-instant-inbound-payment-and-recall-request)
...
## Sending a SCT Inst payment

To send a SCT Inst payment, use the [POST /v1/payments/sepa-instant](#send-a-sepa-instant-payment) endpoint.

You will receive the [SEPA Instant Payment Created webhook](#sepa-instant-outbound-payment-created-webhook). Once the payment has settled, you will receive the [SEPA Instant Outbound Payment Settled webhook](#sepa-instant-outbound-payment-settled-webhook) and the [Customer Accounts Transaction Completed webhook](#customer-accounts-transaction-completed-webhook).

![A message flow diagram explaining how to send a SEPA Instant payment](/assets/images/sepa-ict-send-payment.svg)

<EndpointBlock
  type="post"
  title="Send a SCT Inst payment"
  endpoints={[
    {
...
## Receive a SCT Inst payment

Receipt of a SCT Inst payment can be confirmed by subscribing to the [SEPA Instant Inbound Payment Settled webhook](#sepa-instant-inbound-payment-settled-webhook).

<WebhookPlaceholder render='sepa-instant-inbound-payment-settled-v1' />

## Recall a SCT Inst payment

### Send a recall request

You can send SCT Inst recall requests using the [POST /v1/payments/sepa-instant/{paymentId}/recall](#request-a-sepa-instant-recall) endpoint. You will receive confirmation via the [SEPA Instant Recall Payment Created webhook](#sepa-instant-recall-payment-created-webhook), and a response via either the [Settled](#sepa-instant-recall-payment-settled-webhook), [Rejected](#sepa-instant-recall-payment-rejected-webhook), or [Cancelled](#sepa-instant-recall-payment-cancelled-webhook) webhooks. 

Recall requests remain valid for a maximum of **15 business days**.  If after 15 business days the receiving party has not responded, the recall request is automatically rejected and the [SEPA Instant Recall Request Rejected webhook](#sepa-instant-recall-payment-rejected-webhook) is sent.

![A message flow diagram explaining a successful SCT Inst recall which results in receiving a SEPA Instant Recall Payment Settled webhook](/assets/images/sepa-ict-successful-recall.svg)

![A message flow diagram explaining a rejected SCT Inst recall which results in receiving a SEPA Instant Recall Request Rejected webhook](/assets/images/sepa-ict-rejected-recall.svg)

<EndpointBlock
  type="post"
  title="Recall a SCT Inst payment"
  endpoints={[
    {
...
<EndpointBlock
  type="post"
  title="Respond to a SCT Inst recall request"
  endpoints={[
    {
...
![A message flow diagram explaining responding to a recall request with 'approve' followed by the pacs.004 payment being rejected at RT1 resulting in receiving a SEPA Instant Recall Payment Cancelled webhook](/assets/images/sepa-ict-recall-payment-cancelled.svg)

Following a rejected pacs.004 and receipt of the SEPA Instant Recall Payment Cancelled webhook, the ringfence is removed from the recall amount and you must re-initiate the recall response using the [POST /v1/payments/sepa-instant/{paymentId}/recall-response](#respond-to-a-sepa-instant-recall-request) endpoint.

<WebhookPlaceholder render='sepa-instant-recall-payment-cancelled-v2' />

## Simulate a SCT Inst inbound payment and recall request

<EndpointBlock
  type="post"
  title="Create an inbound SCT Inst payment in the simulation environment"
  endpoints={[
    {
...
<EndpointBlock
  type="post"
  title="Create an inbound SCT Inst recall request in the simulation environment"
  endpoints={[
    {
