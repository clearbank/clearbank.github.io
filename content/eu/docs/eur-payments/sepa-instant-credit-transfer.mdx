...
[SEPA Instant Credit Transfer (SCT Inst)](/eu/docs/api/glossary#sepa-instant-credit-transfe) is a pan-European payment scheme that enables real-time euro transfers, delivering funds to the recipient within 10 seconds, 24 hours a day, 365 days a year.
...
> You are notified of incoming SCT Inst payments by the [SEPA Instant Inbound Payment Settled webhook](#sepa-instant-inbound-payment-settled-webhook) and the [Customer Accounts Transaction Completed webhook](#customer-accounts-transaction-completed-webhook).
...
## SEPA Indirect
SEPA Indirect allows you to participate in SEPA under **your own [BIC](/eu/docs/api/glossary#business-identifier-code) and [IBANs](/eu/docs/api/glossary#international-bank-account-number)**, while connecting to ClearBank through the same APIs you already use. ClearBank Europe (ClearBank NL) acts as your sponsoring direct participant, providing scheme access, clearing, and settlement on your behalf.

You retain full ownership of your customers and payment flows. Send and receive SEPA Credit Transfer (SCT) and SEPA Instant Credit Transfer (SCT Inst) payments with minimal change to your existing integration — ClearBank handles the underlying scheme connectivity so you don't need to become a direct participant yourself.

With SEPA Indirect you get:
- **Your own BIC and IBANs** – maintain your brand identity across all SEPA payments.
- **Full scheme participation** – access SCT and SCT Inst through ClearBank Europe's connectivity.
- **Minimal change to existing flows** – integrate with the ClearBank API and continue operating as you do today.
- **API-first integration** – connect programmatically with a modern, RESTful interface.

<Callout colour="blue">
<p><b>To be eligible for SEPA Indirect, you must:</b><br />
- Be a regulated financial institution (holding a Credit Institution, Electronic Money Institution, or Payment Institution licence).<br />
- Be licensed to operate in a country where ClearBank passports its licence.<br />
- Have, or be in the process of obtaining, your own SWIFT BIC.<br />
- Complete the European Payments Council (EPC) SEPA Adherence process. This is required for both SCT and SCT Inst. </p>
</Callout>

If are interested in becoming a SEPA Indirect participant, contact your Client Director to confirm eligibility and request prerequisite documentation.
...
![A message flow diagram explaining how to send an SCT Inst payment](/assets/images/diagrams/eu/sepa-instant-credit-transfer/sct-inst-send-payment.svg)
...
| MS03        | Payment failed screening, re-send using SCT                             |                             | X                           |
...
<Callout colour="blue">
  If you receive cancellation reason code <strong>MS03</strong>, you should attempt to resend the affected payment using SEPA Credit Transfer instead of SCT Inst. Reattempting the payment using SCT Inst will not succeed.
</Callout>
...
When you call our endpoints in simulation, any IBANs you provide are validated. To support your testing, the following tables list valid and invalid IBANs for your use:
...
You can send SCT Inst recall requests using the [POST /v1/payments/sepa-instant/{paymentId}/recall](#recall-an-sct-inst-payment) endpoint. You will receive confirmation from the [SEPA Instant Recall Payment Created webhook](#sepa-instant-recall-payment-created-webhook), and a response from either the [SEPA Instant Recall Payment Settled](#sepa-instant-recall-payment-settled-webhook) or [SEPA Instant Recall Request Rejected](#sepa-instant-recall-request-rejected-webhook) webhooks. 
...
![A message flow diagram explaining a successful SCT Inst recall which results in receiving a SEPA Instant Recall Payment Settled webhook](/assets/images/diagrams/eu/sepa-instant-credit-transfer/sct-inst-successful-recall.svg)

![A message flow diagram explaining a rejected SCT Inst recall which results in receiving a SEPA Instant Recall Request Rejected webhook](/assets/images/diagrams/eu/sepa-instant-credit-transfer/sct-inst-rejected-recall.svg)
...
<Callout colour="blue">
If `approve` is set to `true`, a [pacs.004](/eu/docs/api/technical-reference#pacs-004) payment will be automatically initiated. You do not need to initiate a payment.
If `approve` is set to `false`, the `rejectionCode` field must be populated with a rejection reason code.
</Callout>

Following your approval of a recall request, the amount of the original payment is ring-fenced and returned to the sending party. You can be notified of the status of the status of the return payment by the [SEPA Instant Recall Payment Created webhook](#sepa-instant-recall-payment-created-webhook), [SEPA Instant Recall Payment Settled webhook](#sepa-instant-recall-payment-settled-webhook), and [SEPA Instant Recall Payment Cancelled webhook](#sepa-instant-recall-payment-cancelled-webhook).
...
![A message flow diagram explaining receiving a recall request, being notified by the SEPA Instant Recall Request Received webhook, responding with 'approve' set to 'true' using the POST /v1/payments/sepa-instant/{paymentId}/recall-response endpoint and receiving the SEPA Instant Recall Payment Created webhook and SEPA Instant Recall Payment Settled webhook following the return of the recalled amount.](/assets/images/diagrams/eu/sepa-instant-credit-transfer/sct-inst-receive-recall-approved.svg)

![A message flow diagram explaining receiving a recall request, being notified by the SEPA Instant Recall Request Received webhook and responding with 'approve' set to 'false' with a valid 'rejectionCode' using the POST /v1/payments/sepa-instant/{paymentId}/recall-response endpoint](/assets/images/diagrams/eu/sepa-instant-credit-transfer/sct-inst-receive-recall-rejected.svg)
...
![A message flow diagram explaining responding to a recall request with 'approve' followed by the pacs.004 payment being rejected at RT1 resulting in receiving a SEPA Instant Recall Payment Cancelled webhook](/assets/images/diagrams/eu/sepa-instant-credit-transfer/sct-inst-recall-payment-cancelled.svg)
