...
You can:
- [Send a SCT Inst payment](#send-a-sct-inst-payment)
- [Receive SCT Inst payment](#receive-a-sct-inst-payment)
- [Recall a SCT Inst payment](#recall-a-sct-inst-payment) (integrating with recall payments is required)
- [Receive and respond to a recall request](#receive-and-respond-to-a-recall-request)
- [Simulate a SCT Inst inbound payment and recall request](#simulate-a-sct-inst-inbound-payment-and-recall-request)
> You are notified of incoming SCT Inst payments via the [SEPA Instant Inbound Payment Settled webhook](#sct-inst-inbound-payment-settled-webhook) and the [Customer Accounts Transaction Completed webhook](#customer-accounts-transaction-completed-webhook).
...
To send a SCT Inst payment, use the [POST /v1/payments/sepa-instant](#send-a-sct-inst-payment) endpoint.
...
![A message flow diagram explaining how to send a SEPA Instant payment](/assets/images/sct-inst-send-payment.svg)
...
To respond to the recall request, use the [POST /v1/payments/sepa-instant/{paymentId}/recall-response](#respond-to-a-sct-inst-recall-request) endpoint. Set the `approve` boolean `true` to accept the request or `false` to reject it.
...
Following a rejected pacs.004 and receipt of the SEPA Instant Recall Payment Cancelled webhook, the ringfence is removed from the recall amount and you must re-initiate the recall response using the [POST /v1/payments/sepa-instant/{paymentId}/recall-response](#respond-to-a-sct-inst-recall-request) endpoint.
