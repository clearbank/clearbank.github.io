...
Cancelled outbound payments can return a reason code which will be reflected in the `CancellationCode` field of the [SEPA Instant Outbound Payment Cancelled webhook](#sepa-instant-outbound-payment-cancelled-webhook). The cancellation codes include, but are not limited to:
...
| RR03 | Missing Creditor’s Name Or Address |
