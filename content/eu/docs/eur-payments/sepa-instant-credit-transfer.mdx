...
<Callout colour="blue">
  When a payment is delayed and then settles, the TransactionId will change. You can instead use the PaymentId for an identifier that remains unchanged throughout the payment's lifecycle.
</Callout>
