...
[SEPA Credit Transfer (SCT)](/eu/docs/api/glossary#sepa-credit-transfer) is a payment scheme that enables you to move euros between scheme participants. Transactions are capped at €999,999,999.99 per payment.

You can:
- [Send an SCT payment](#send-an-sct-payment)
- [Recall an SCT payment](#recall-an-sct-payment)
- [Return an SCT payment](#return-an-sct-payment) (Integrating with recall and return is required.)
- [Receive and respond to an SCT recall request](#receive-and-respond-to-an-sct-recall-request)
...
> You will be notified of incoming SCT payments by the [SEPA CT Inbound Payment Completed](#sepa-ct-inbound-payment-completed-webhook) webhook.
...
![A message flow diagram explaining how to send an SCT payment](/assets/images/diagrams/eu/sepa-credit-transfer/sepa-ct-send-payment.svg)
...
When a payment fails, you'll receive an [Outbound Payment Failed webhook](#sepa-ct-outbound-payment-failed-webhook), which includes a `reason` code specifying the reason for failure. The failure codes include, but are not limited to:
...
When you call our endpoints in simulation, any IBANs you provide are validated. To support your testing, refer to the following tables of valid and invalid IBANs:
...
* If your recall request has been **accepted**, you will be notified by the [SEPA CT Outbound Payment Return Completed](#sepa-ct-outbound-payment-return-completed-webhook) and [Customer Accounts Transaction Completed](#customer-accounts-transaction-completed-webhook) webhooks.

* If your recall request has been **rejected**, you will be notified by the [SEPA CT Outbound Payment Recall Rejected](#sepa-ct-outbound-payment-recall-rejected-webhook) webhook.

![A message flow diagram explaining how to an SCT recall request is sent, then accepted or rejected by the original creditor](/assets/images/diagrams/eu/sepa-credit-transfer/sepa-ct-recall-request.svg)
...
You may receive a recall request from the [SEPA CT Inbound Payment Recalled webhook](#sepa-ct-inbound-payment-recalled-webhook). You can review the details of the request by referring to the 'PaymentId' for details of the original payment received and 'CancellationReasonInformation' for the reason the sending party is requesting the recall (see [recall request reason codes](#recall-request-reason-codes)). 

To respond to the recall request, use the [/v1/payments/sepa/{paymentId}/recall-response](#respond-to-an-sct-recall-request) endpoint. Recall requests should be responded to within 15 business days. If after 15 business days you do not accept or reject the recall, ClearBank will automatically reject the request.

#### Accept recall requests
You can accept the recall request by setting the `RecallResponseAccepted` boolean to `true`.

![A message flow diagram showing how to accept an inbound SCT recall request](/assets/images/diagrams/eu/sepa-credit-transfer/sepa-ct-recall-response-accepted.svg)

<Callout colour="blue">
<p>If <b>RecallResponseAccepted</b> is set to <b>true</b>, a [pacs.004](/eu/docs/api/technical-reference#pacs-004) payment will be automatically initiated. You do not need to initiate a payment.</p>
</Callout>

#### Reject recall requests
If you do not agree with the reason for the recall cited by the requesting party in the `CancellationReasonInformation` field, you can reject the recall request by setting the `RecallResponseAccepted` boolean to `false`. When rejecting a recall request, the 'RejectCode' field must be populated (see [recall response reason codes](#recall-response-reason-codes)).

![A message flow diagram showing how to reject an inbound SCT recall request](/assets/images/diagrams/eu/sepa-credit-transfer/sepa-ct-recall-response-rejected.svg)

<Callout colour="blue">
<p>If <b>RecallResponseAccepted</b> is set to <b>false</b>, the <b>rejectionCode</b> field must be populated with a rejection reason code.</p>
</Callout>
...
![A message flow diagram explaining how to send an SCT return](/assets/images/diagrams/eu/sepa-credit-transfer/sepa-ct-return.svg)
...
You will be notified of a successful SCT return payment by the [SEPA CT Outbound Payment Return Completed webhook](#sepa-ct-outbound-payment-return-completed-webhook).

<WebhookPlaceholder render='sepa-ct-outbound-payment-return-completed-v1' />

## Simulate SCT payment scenarios

The following lets you test SCT payment scenarios in the ClearBank simulation environment.
