...
To respond to the recall request, use the [/v1/payments/sepa/{paymentId}/recall-response](#respond-to-an-sct-recall-request) endpoint. 
...
<Callout colour="blue">
If `RecallResponseAccepted` is set to `true`, a pacs.004 payment will be automatically initiated. You do not need to initiate a payment. If `RecallResponseAccepted` is set to `false`, the `rejectionCode` field must be populated with a rejection reason code.
</Callout>
