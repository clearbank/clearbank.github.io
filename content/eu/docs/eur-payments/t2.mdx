...
T2 has no transaction limit, making it suitable for high-value payments. Transactions are processed in euro using central bank money. When you submit a payment, we send a settlement request to the RTGS system. This processes the request and, upon approval, debits ClearBank and credits the recipient bank. It also sends a settlement confirmation.

<br/>

![T2 message flow diagram](/assets/images/Target2-message-flow.svg)

**Customer payments** are payments that have either your or another financial institution's customer as the ultimate debtor or creditor. **Institution payments** are exclusively financial institution (FI) to financial institution. **Return payments** can be returned to either your customer or a financial institution.

Institution (pacs.009) payments are available to our clients for FI-to-FI payments only. If you attempt to send a pacs.009 payment to an end-user account, the payment will be returned. You will still be charged for the payment.
