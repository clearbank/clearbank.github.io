...
Institution (pacs.009) payments are available to our clients for FI to FI payments. If you attempt to send a pacs.009 payment to an end-user account, the payment will be returned as a pacs.004. You will receive the [T2 Payment Return Outbound Completed webhook](#t-2-payment-return-outbound-completed-webhook) and will still be charged for the payment.
