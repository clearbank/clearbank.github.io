...
[T2](/eu/docs/api/glossary#t2), formerly known as TARGET2, is the [Real-Time Gross Settlement (RTGS)](/eu/docs/api/glossary#real-time-gross-settlement) payment scheme for euro payments. It uses [ISO 20022](/eu/docs/api/technical-reference#iso-20022) message formats.
...
![T2 message flow diagram](/assets/images/diagrams/eu/t2/Target2-message-flow.svg)
...
You can send [pacs.008](/eu/docs/api/technical-reference#pacs-008) and [pacs.004](/eu/docs/api/technical-reference#pacs-004) payments Monday - Friday, 07:00 - 17:00 CET, with pacs.009 extended until 18:00 CET. Outside of these operating hours, outgoing payments will be rejected.
...
![T2 send a payment message flow diagram](/assets/images/diagrams/eu/t2/t2-send-payment.svg)
...
### T2 outbound failure error codes

The following error codes may appear in the `Errors` field of the T2 failure webhooks listed, helping you understand why a T2 payment was unsuccessful:

- [T2 Customer Credit Transfer Outbound Failed webhook](#t-2-customer-credit-transfer-outbound-failed-webhook)
- [T2 Financial Institution Credit Transfer Outbound Failed webhook](#t-2-financial-institution-credit-transfer-outbound-failed-webhook)
- [T2 Payment Return Outbound Failed webhook](#t-2-payment-return-outbound-failed-webhook)

| Error code | Description |
|------------|-------------|
| AC01       | Incorrect account number (IBAN or BIC not found on ClearBank platform) |
| AC04       | Closed account |
| AC06       | Account is suspended |
| AGNT       | Creditor Agent BIC not found |
| AM03       | IBAN doesn’t support Euro |
| AM04       | Insufficient Funds |
| BE01       | IBAN or BIC not linked to institution ID |
| BE06       | FI Agent Client BIC not linked to IBAN |
| CB01       | Unknown Error |
| CB02       | Return request amount exceeds original amount |
| CB03       | Original payment must be settled |
| CB04       | Partial returns not supported |
| CNOR       | Creditor Bank not registered/routable through T2 |
| RC05       | Source BIC not found on CB Platform |
| RC11       | Intermediary Agent BIC not found |
...
![T2 return a payment message flow diagram](/assets/images/diagrams/eu/t2/t2-return-payment.svg)
...
![T2 send an FI payment message flow diagram](/assets/images/diagrams/eu/t2/t2-send-FI-payment.svg)
...
When an inbound T2 payment is received, you'll be notified with one of the following webhooks depending on the payment type.

For institution payments ([pacs.009](/eu/docs/api/technical-reference#pacs-009)), the T2 scheme does not require IBANs to be provided. **We direct any inbound institution payments addressed only to a BIC and not with an IBAN to your** [**Primary Operating Account**](../accounts/account-types) (this is the first Operating Account created).
...
These simulation endpoints are comprehensive in the fields they allow you to test, however for simple test scenarios such as funding an account you can use the minimum set of required fields. 

We've provided an example payload that you can copy and paste. **You may need to edit certain fields.**

#### Submit an inbound T2 payment (pacs.008) endpoint example

**In this example, you'll need to edit xyz field**

```
{
    "paymentIdentification": {
        "instructionId": "75030e66dc7844ed9e4a1266c9e8410f",
        "endToEndId": "Inbound Payment",
        "uetr": "3f2504e0-4f89-11d3-9a0c-0305e82c3301"
    },
    "header": {
        "bizMsgId": "75030e66dc7844ed9e4a1266c9e8410f",
        "from": "CLRBNL2AXXX",
        "to": "CLRBNL2AXXX"
    },
    "value": {
    ...
    "creditor": {
        ...
        "account": {
            "identification": {
                "iban": "INSERT CREDITOR IBAN"
            }
        }
    },
    "creditorAgent": {
        "identification": {
            "bicFi": "INSERT CREDITOR BIC"
        }
    },
    "instructingAgent": {
        "bic": "CLRBNL2AXXX"
