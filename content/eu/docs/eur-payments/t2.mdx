...
**Customer payments** - also known as pacs.008 payments - are payments that have either your or another Financial Institution's (FI) customer as the ultimate debtor or creditor.

**Return payments** - also known as pacs.004 payments - are payments returned to either your customer or an FI.

**Institution payments** - also known as pacs.009 payments - are exclusively for FI to FI transactions.

* If you attempt to send an institution payment to an end-user account, the payment will be returned. However, you'll still be charged a processing fee for the payment.
* While we do require you to provide an IBAN for [outbound institution payments](#sending-a-t2-payment-to-an-institution), the T2 scheme does not strictly require it. For this reason, any inbound institution payments we receive that do **not** include an IBAN will be directed to your [**Primary Operating Account**](../accounts/account-types.mdx).
