...
SEPA CT (SCT) is a payment scheme that enables you to move euros between scheme participants. Transactions are capped at €999,999,999 per payment.

You can:
- [Send an SCT payment](#sending-an-sct-payment)
...
- [Respond to an SCT recall requests](#respond-to-an-sct-recall-request)
- [Receive an SCT payment](#receive-an-sct-payment)
- [Receive an SCT return payment](#receive-an-sct-return-payment)
...
You can send an SCT recall requests using the [POST /payments/sepa-ct/v1/recall](#request-a-sepa-ct-payment-recall) endpoint. Recalls can take up to 15 business days.
...
| TECH   | Technical problems resulting in an erroneous SEPA Instant Transaction | 10 business days                  |
...
![A message flow diagram explaining how to send an SCT recall request](/assets/images/sepa-ct-recall-response.svg)
...
### Simulate an SCT return payment

To simulate the return of an SCT payment in the simulation environment, use the [Send a SEPA CT payment](#send-a-sepa-ct-payment) endpoint with the `remittanceInformationUnstructured` field populated with `--SCTRETURN--`. This will simulate an outbound SCT payment settling and then being returned by the creditor.
