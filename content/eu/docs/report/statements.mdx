...
Statements can be generated either for all your accounts, or for a specified real account. The statement will be provided by a webhook (refer to [Use webhooks](../api/getting-started/#use-webhooks)).
...
You can request a statement containing all payment activity across your ClearBank accounts for a completed business day, also referred to as a 'Bank to Customer Statement'. The statement is delivered in an [ISO 20022](/eu/docs/api/technical-reference#iso-20022) compliant XML file known as camt.053 (Cash Management message).
...
### Time zone examples
The following examples highlight the differences in startDateTimeUtc and endDateTimeUtc depending on the current time zone (CET or CEST) including specific examples of the first day of a new time zone. See the diagram for a visual representation of these changes.

In these examples, the following parameters remain constant:
- ClearBank completes its reconciliation based on the current Europe/Amsterdam timezone
- The endpoint request must be in UTC time

|Scenario|startDateTimeUtc|endDateTimeUtc|Notes|
|--------|----------------|--------------|-----|
|**Requesting a statement in winter**<br /> It's 5 February 2024 <br /> I want to request yesterday's statement (4 February 2024) <br /> Yesterday and today's time zone: CET (UTC+**1 hour**) |2024-02-04T23:00:00Z|2024-02-05T23:00:00Z|CET is 1 hour ahead of UTC, therefore the times in the request must each be reduced by **1 hour**.|
|**Requesting a statement in summer**<br /> It's 5 July 2024 <br /> I want to request yesterday's statement (4 July 2024) <br /> Yesterday and today's time zone: CEST (UTC+**2 hours**) |2024-07-03T22:00:00Z|2024-07-04T22:00:00Z|CEST is 2 hours ahead of UTC, therefore the times in the request must each be reduced by **2 hours**.|
|**Requesting a statement after the time zone changes from CET to CEST**<br /> It's 1 April 2024 <br /> I want to request yesterday's statement (31 March 2024) <br /> Yesterday's time zone: CET (UTC+**1 hour**) and today's time zone: CEST (UTC+**2 hours**) |2024-03-30T23:00:00Z|2024-03-31T22:00:00Z|When the time zone changes from CET to CEST on 31 March, the clocks go forward 1 hour making it a **23-hour day**.|
|**Requesting a statement after the time zone changes from CEST to CET**<br /> It's 2 November 2024 <br /> I want to request yesterday's statement (1 November 2024) <br /> Yesterday's time zone: CEST (UTC+**2 hour**) and today's time zone: CET (UTC+**1 hour**) |2024-10-31T22:00:00Z|2024-11-02T00:00:00Z|When the time zone changes from CEST to CET on 1 November, the clocks go back 1 hour making it a **25-hour day**.<br /> This means the start and end date range of the request is from 31 October to 2 November.|


<br />
<br />

![Image showing when the time zone changes from CET to CEST and the effect the time zone change has on the 24-hour clock.](/assets/images/diagrams/eu/statements/camt053-time-zone-eu-graphic.svg "time zones for camt.053 statement requests")
...
![Image showing camt.053 statement message flow](/assets/images/diagrams/shared/account-reporting/camt053-message-flow.svg "camt.053 statement request message flow")
...
<EndpointBlock
  type="post"
  title="Request a camt.053 statement for all accounts"
  endpoints={[
    {
...
|UETR |String |1 |36 |+Stmt<br /> ++Ntry<br /> +++NtryDtls<br /> ++++TxDtls<br /> +++++Refs<br /> ++++++UETR |Optional |Universally unique identifier to provide an end-to-end reference of a payment transaction. |
