...
We use [webhooks](/eu/docs/api/technical-reference#webhook) to deliver transaction confirmation notifications and account-specific reconciliations to your system as soon as such events occur on your account. Please refer to [Set up and test webhook listener URLs](#set-up-and-test-webhook-listener-urls) for more information.
...
Refer to the below table for definitions of the retry reasons that may be included in the `x-retry-reason` header for reattempted webhook.
