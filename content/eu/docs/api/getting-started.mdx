...
Before you can integrate with [the ClearBank API](/eu/docs/api/overview), you need to onboard with us as a customer and must have a valid authentication profile.
...
<Callout colour="blue">
  <strong>Security notice: Protecting your credentials</strong><br/><br/>
  To access the ClearBank API, use sensitive credentials including API tokens, private keys and digital certificates. These credentials grant direct access to your data and must be protected at all times. You must store, transmit and manage these credentials securely.<br/><br/>
  Credentials must never be shared and may expire periodically. You are responsible for renewing them promptly. Any suspected compromise must be reported to ClearBank immediately, as exposure of credentials could allow unauthorised access to your ClearBank services.
</Callout>
...
- A [certificate signing request (CSR)](/eu/docs/api/technical-reference#certificate-signing-request) in [PKCS](/eu/docs/api/technical-reference#public-key-cryptography-standards) #10 format*
- A token
...
> In the simulation environment, you can utilise open source tools to store your private key. In the production environment, you must use a physical or virtual HSM solution to store your private key (see HSM requirements).
...
4. Sign in to the ClearBank Portal using the URL from the table.
5. From the menu, select **Institution > Certificates and Tokens**.
6. Select **Generate API Token**.
> ![alt text](/assets/images/diagrams/shared/getting-started/GenerateToken_1.png "Certificates and Tokens page")
...
>> ![alt text](/assets/images/diagrams/shared/getting-started/GenerateToken_2.png "Generate API tokens")
...
>> ![alt text](/assets/images/diagrams/shared/getting-started/GenerateToken_3.png "View Tokens")

Congratulations! You have successfully created your authentication profile.

### Create your digital signature

We use a [digital signature](/eu/docs/api/technical-reference#digital-signature) to ensure that the integrity of a request body (sent to us) is maintained. The whole HTTP request body (in its original format) is used to create a digital signature, as it is specific to the body of each request.
...
1. UTF-8 encode the exact request body that will be sent to the ClearBank API (any differences including the addition or removal of spaces will result in a totally different digital signature).
...
5. Send the Base64 encoded string representation of the digital signature in the HTTP header [**DigitalSignature**](/eu/docs/api/technical-reference#digital-signature) for all requests that have a request body (POST, PUT, and PATCH).
...
For additional help with creating your authentication profile and digital signature, refer to the following examples on the ClearBank GitHub repository:
...
Note that the code in these examples is not production ready and has been simplified for brevity.

## Use endpoints

Once you have set up your connection to our simulation environment, you are ready to send requests using the ClearBank API endpoints. The information here allows you to use the GET /v1/Test and POST /v1/Test endpoints.

### Header parameters

We utilise HTTP-request headers for the ClearBank API calls which include `Authorization`, `DigitalSignature`, `X-Request-Id`, and `X-Request-Time` headers. In all our responses we will send an `X-Correlation-Id` in the header, together with an [HTTP status code](#http-status-codes).
...
Your API token should be included in your HTTP `Authorization` header. Refer to [Header parameters](#header-parameters) for more information.
...
We use [webhooks](/eu/docs/api/technical-reference#webhook) to deliver transaction confirmation notifications and account-specific reconciliations to your system as soon as such events occur on your account. Refer to [Set up and test webhook listener URLs](#set-up-and-test-webhook-listener-urls) for more information.
...
| `503`       | **Server error** - Service unavailable (see [5XX status codes](#5-xx-status-codes)).                                                                                    |
...
See the following examples of some common 4XX responses and their meaning which may assist you in troubleshooting.
...
In exceptional circumstances where the initial request was effectively processed despite returning a `5XX` status, your second request would make that clear either in the response or through an appropriate webhook. 

In the event a `5XX` response is caused by a planned or unexpected system outage on our part, we will communicate this by email.
...
```
### Rate limiting
We use [rate limiting](/eu/docs/api/technical-reference#rate-limit) to protect our systems from excessive load and to ensure a consistent level of availability and performance to all our clients.
...
We use HAL (Hypertext Application Language) to hyperlink between resources in the ClearBank API.
...
| `templated` | `boolean` |          | Specified as `true` if href is a URI template with parameters. Otherwise, this property is absent or set to `false` |
...
| Element              | Body/Header | Type   | Description                                                                                                                                                             |
| -------------------- | ----------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `x-delivery-attempt` | Header      | string | Starting from 1 with the original webhook, increments with each retry attempt. Should not be used to assure idempotency.                                                |
| `x-retry-reason`     | Header      | string | If this is a retry, the reason for the retry attempt.                                                                                                                   |
| `DigitalSignature`   | Header      | string | ClearBank's Digital Signature                                                                                                                                           |
| `Type`               | Body        | string | The type of the event                                                                                                                                                   |
| `Version`            | Body        | int    | The webhook version                                                                                                                                                     |
| `Nonce`              | Body        | int    | Cryptographically secure number generated by ClearBank for every single webhook. A nonce number is generated randomly and should not be used as a check for duplication |
| `Payload`            | Body        | object | Object which contains information related to the webhook                                                                                                                |

##### Retry reason table

Refer to the following table for definitions of the retry reasons that may be included in the `x-retry-reason` header for reattempted webhook.

| Enum value                    | Reason                                             |
| ----------------------------- | -------------------------------------------------- |
| `"Connection"`                | HTTP Connection errors such as DNS/SSL failures.   |
| `"Timeout"`                   | Webhook endpoint did not respond within 5 seconds. |
| `"HTTP"`                      | Webhook endpoint returned 4xx/5xx instead of 200.  |
| `"Non-repudiation"`           | Webhook response signature could not be verified.  |
| `"Payload / Response Format"` | Webhook response format was invalid                |

**Note:** The `x-retry-reason` header is only present when we are unable to verify successful delivery and retry sending the webhook. It is not present on the first webhook attempt, or in duplicate attempts. You should therefore not rely on this header to detect duplicates. Instead, you should check for duplicates based on the webhook's `Payload`.
...
> Your private key needs to correspond to a public key which has been used to create a certificate signing request (CSR). Refer to [create your authentication profile](#create-your-authentication-profile) for more information.
...
You must be able to handle duplicate webhooks [idempotently](/eu/docs/api/technical-reference#idempotency). This means you should always respond in the appropriate format even if you have detected a duplicated or retried webhook. If you don't, we won't be able to verify the webhook was successfully received and will therefore retry delivery.
...
>> ![alt text](/assets/images/diagrams/shared/getting-started/webhook-management-page.png "Webhook management page")
