...
A refund can occur when a settled SEPA Direct Debit is returned to the debtor’s account after settlement. This can happen for various reasons, such as a dispute raised by the debtor or a refund request initiated by the creditor.
...
Use reasonCode `MD06` to simulate a refund request initiated by the creditor within the first 8 weeks of settlement. Use reasonCode `MD01` to simulate a refund due to an illegitimate mandate (triggered between 8 weeks and 13 months after settlement).
