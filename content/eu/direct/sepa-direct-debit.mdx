...
A refund is a post settlement R-flow where funds from a SEPA Direct Debit are returned to the debtor's account following request from the debtor. This can happen for various reasons, such as a dispute raised by the debtor or a withdrawal of consent (end customer).
...
Use reasonCode `MD06` to simulate a refund request initiated by the debtor within the first 8 weeks of settlement. Use reasonCode `MD01` to simulate a refund due to no valid mandate exists (triggered between 8 weeks and 13 months after settlement).
