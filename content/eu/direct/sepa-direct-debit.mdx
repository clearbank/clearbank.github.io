...
<p>All information on this page describes a solution that is <strong>not yet live in production and therefore could change</strong>. This includes any endpoint and webhook details. The information is being provided early to aid you in integrating with the ClearBank API in our EU simulation environment.</p>
</Callout>

[SEPA Direct Debit (SDD)](/eu/docs/api/glossary#sepa-direct-debit) is a pan-European payment scheme that enables organisations to collect euro-denominated payments from a payer’s account, provided the payer has authorised the collection in advance. The scheme is defined and maintained by the <ExternalLink href="https://www.europeanpaymentscouncil.eu/what-we-do/sepa-direct-debit">European Payments Council (EPC)</ExternalLink> and is available across SEPA-participating countries.
...
An [SDD mandate](/eu/docs/api/glossary#direct-debit-mandate) is the payer’s authorisation for a creditor to initiate direct debit collections against the payer’s account. A mandate provides a legal basis for each collection submitted under the scheme.
...
3. **Settlement**  
   If successfully processed, the debit is applied to the debtor account and funds are settled to the creditor by the scheme.
...
![A message flow diagram showing which responses and webhooks you will receive after requesting to simulate an inbound direct debit.](/assets/images/diagrams/eu/sepa-direct-debit/sepa-dd-flow.svg)
...
ClearBank executes refund requests submitted through the API and forwards them to the scheme. ClearBank does not assess or determine the validity of the underlying dispute.
...
Reversals occur within a short scheme-defined timeframe following settlement (up to 5 business days) and result in the original debit being reversed and funds credited back to the debtor account.

#### Associated Reversal webhooks

- **[SEPA DD Debit Payment Reversal Completed](#sepa-dd-debit-payment-reversal-completed-webhook)**: sent when a reversal instruction has been successfully processed after settlement. This webhook confirms that the reversal instruction has been accepted and applied, resulting in the previously settled debit being reversed. Once received, the reversal is considered finalised and the corresponding funds have been credited back to the debtor account.
- **[SEPA DD Debit Payment Reversal Failed](#sepa-dd-debit-payment-reversal-failed-webhook)**: sent when a reversal instruction cannot be successfully processed. This indicates that the reversal instruction has failed. In such cases, the original settled debit remains unchanged, and no funds are returned to the debtor account.
...
<WebhookPlaceholder render='sepa-dd-debit-payment-reversal-completed-v1' />

<WebhookPlaceholder render='sepa-dd-debit-payment-reversal-failed-v1' />
