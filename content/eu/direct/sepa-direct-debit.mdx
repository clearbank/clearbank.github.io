...
Use reasonCode `MD06` to simulate a refund request initiated by the debtor within the first 8 weeks of settlement. Use reasonCode `MD01` to simulate an unauthorised refund where no valid mandate exists, which can be requested from 8 weeks up to 13 months after settlement. These timeframes are only applicable in our EU production environment: you can simulate a refund of either type at any time in our EU simulation environment.
