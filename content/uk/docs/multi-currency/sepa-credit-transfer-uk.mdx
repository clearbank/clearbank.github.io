...
When you send several payments at once in bulk, each payment is processed individually. This means if one payment fails, the remainder of the batch will be unaffected. You'll receive either a [Cross-Border Outbound Payment Failed](#cross-border-outbound-payment-failed-webhook), or a [Cross-Border Outbound Payment Completed](#cross-border-outbound-payment-completed-webhook) and a [Customer Accounts Transaction Completed](#sct-uk-customer-accounts-transaction-completed-webhook) webhook for each payment in the batch, so you can keep track of their status.
