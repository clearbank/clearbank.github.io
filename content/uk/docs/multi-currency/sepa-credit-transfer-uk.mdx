...
To simulate the return of an SCT UK payment in the simulation environment, use the [Send an SCT payment](#send-an-sct-payment) endpoint with the `remittanceInformation.unstructured` field populated with `--SCTRETURN--`. This will simulate an outbound SCT UK payment settling and then being returned by the creditor.
