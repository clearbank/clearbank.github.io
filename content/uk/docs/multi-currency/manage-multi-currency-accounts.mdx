...
**Before creating an account, ensure that you know which [account type](./multi-currency-account-types) you need to create.** If you are unsure, contact us.

You can use a multi-currency account to hold multiple non-GBP currency balances under the same IBAN, which can be specified at the time of creating the account using the ClearBank API. Prior to assigning your desired IBAN to the account, we will validate it to ensure that the IBAN format is acceptable and not already in use.
...
For more information about currencies supported by ClearBank multi-currency accounts, speak to your Relationship Manager.
...
Multi-currency virtual accounts are only compatible with General Client Accounts and General Segregated Accounts, and can be created, suspended or activated and closed using the ClearBank API. 
