...
Our [Foreign Exchange (FX)](/uk/docs/api/glossary#foreign-exchange) trade offering lets you buy and sell currency in real-time. You can submit a trade order to ClearBank and we perform the required validation checks on your order. Upon successful validation, it is executed in the foreign exchange market against live market rates.

You can use our [FX trade](/uk/docs/api/glossary#fx-trade) functionality to perform FX trades between accounts which belong to the same owner. For example, let’s assume that you want to sell GBP from Account A to purchase EUR for Account B – in doing so, you will need to ensure that the owner of both accounts is the same. This applies to two accounts that belong to you, that is, the financial institution or your customers.

When initiating a FX trade order using the ClearBank API, you can either specify the amount of the currency you wish to buy or the amount you wish to sell. The specified amount (and currency) is referred to as the fixed side of the trade. Whereas the amount (and currency) that has not been specified is referred to as the floating side of the trade.
...
FX trades are subject to the cut-off times of both currencies in the trade. To execute a same-day trade, both currencies must be within their operating hours. A trade scheduled for next day or 2 days' time (indicated by a `valueDate` of T+1/T+2) can be executed for all currencies up to 23:30.
...
For further details on cut-off times, refer to the following table detailing the cut-off times for all currently supported currencies. Note that all times are in **UK time**.
