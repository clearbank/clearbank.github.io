...
A [Request for Quote (RFQ)](/uk/docs/api/glossary#request-for-quote) in the context of foreign exchange (FX) will allow you to request a price up-front for any of our currently supported currency pairs and hold that rate for a set period. 

Once you receive the response with the quote, the rate is held for **45 seconds**. If you decide that you want to initiate the trade, a separate trade request must be submitted within the held rate period. This functionality enhances your trading capabilities by providing a transparent and efficient way to secure the held rates by ClearBank.
...
Refer to the following table detailing the cut-off times for all currently supported currencies. Note that all times are in **UK time**.
...
For more information about cut-off times, speak to your Client Director.
</Callout>

<br/>

<Callout colour="blue">
Due to public holidays in the US, pricing of all currency pairs for our FX services will be limited or unavailable on the following dates:<br/>

•	**Monday 7th September (Labor Day)** <br/>
•	**Monday 12th October (Columbus Day)** <br/>
•	**Wednesday 11th November (Veterans Day)** <br/>
•	**Thursday 26th November (Thanksgiving Day)** <br/>
•	**Friday 25th December (Christmas Day)** <br/>

While same day trades may be unavailable on these dates, TOM/SPOT trades will be available to book. Be aware of TOM/SPOT trades booked in the days leading up to these dates.
</Callout>

### FX trades minimum values

FX trades have minimum trading values defined for each currency. If you try to request a trade below these values, you will receive a 400 error response. For the full list of minimum trade values per currency, see [Foreign exchange trade minimum values](./fx-trade#fx-trade-minimum-values).

### FX trades maximum size

Each FX currency pair has a defined maximum trade size per request. If you request a trade exceeding this limit, you'll receive an error response. The following table outlines the maximum allowable trade size for each currency pair:
...
| EUR/NOK       | 5,000,000 EUR      |
| EUR/SEK       | 5,000,000 EUR      |
| EUR/USD       | 15,000,000 EUR     |
| GBP/CAD       | 10,000,000 GBP     |
...
| USD/SEK       | 5,000,000 USD      |

Maximum trade sizes are under regular review and are subject to change as we work to extend the limits based on demand. For more information about maximum trade sizes, contact your Client Director.
