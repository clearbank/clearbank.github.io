...
When you receive an inbound payment, you'll be notified with either a [Multi-currency Transaction Settled webhook](#multi-currency-transaction-settled-webhook), or a [Cross-Border Inbound Payment Completed webhook](#cross-border-inbound-payment-completed-webhook) - depending on if the payment was received via SEPA CT.
