...
When you receive an inbound payment, you'll be notified with either:
* A [Multi-currency Transaction Settled webhook](#multi-currency-transaction-settled-webhook) for a standard MCCY payment.
* A [Cross-Border Inbound Payment Completed webhook](#cross-border-inbound-payment-completed-webhook) for any payment received via SEPA CT UK.
