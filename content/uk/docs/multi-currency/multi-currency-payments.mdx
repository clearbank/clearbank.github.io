...
ClearBank accepts inbound and outbound EUR payments through the payment schemes **T2** and **SEPA Credit Transfer (SCT)**. For more details about EUR payments processed by SCT, refer to [SEPA Credit Transfer UK](./sepa-credit-transfer-uk).
...
For up to date information on the countries you can send high value multi-currency payments to and the available currencies ClearBank supports (and their respective cut-off times), speak with your Client Director.
...
<Callout colour="blue">
  Currency availability is affected by both UK bank holidays and the holidays observed in each currency's country. For more information about currency availability, speak to your Client Director.
</Callout>
...
* A [Multi-currency Transaction Created webhook](#multi-currency-transaction-created-webhook) for all MCCY payments, including EUR through SCT. 
* A [Multi-currency Transaction Settled webhook](#multi-currency-transaction-settled-webhook) for all MCCY payments, including EUR through SCT.
