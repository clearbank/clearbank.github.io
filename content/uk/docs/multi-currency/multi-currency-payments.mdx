...
* A [Cross-Border Inbound Payment Completed webhook](#cross-border-inbound-payment-completed-webhook) for EUR payments received via SCT only.

If you receive a EUR payment via SCT, you'll be notified via all three webhooks. To support your idempotency logic, you can compare the `TransactionId` field present in each webhook: this will always have the same value when referring to the same payment.
