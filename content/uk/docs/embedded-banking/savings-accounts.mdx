...
You can update the interest configuration of your savings accounts yourself.

During account creation, you set your initial interest configuration. You can have multiple interest configurations using different configuration IDs. You can request a new configuration ID from your usual ClearBank contact. 

To change the interest configuration, call the [POST /interest/v1/accounts/{accountId}/configurations](#update-interest-configuration) endpoint with your new interest rate ID in the `InterestConfigurationId` field. 
