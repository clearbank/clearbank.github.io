...
ClearBank also supports hub accounts. A hub account is a type of savings account with bespoke restrictions and privileges set by prior arrangement with ClearBank. If you have a use case for savings accounts that doesn't quite fit, ask your Client Director about hub accounts.
...
### Savings and hub account usage guide

**To create a savings or hub account:**
...
2. Use the [POST /v2/savings](#create-a-savings-account) (or [POST /accounts/sterling/v1/hub-accounts](#create-a-hub-account) for hub accounts) endpoint to create the account on behalf of the customer.
...
Hub accounts should only be used by prior arrangement with ClearBank. They exist to facilitate innovative financial products that require more flexibility than standard savings accounts. Hub accounts are usually restricted in one or more of the following ways:
