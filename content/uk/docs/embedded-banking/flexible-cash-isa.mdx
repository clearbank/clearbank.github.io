...
Customers can invalidate their cash ISA subscriptions in the following ways:

* Moving address out of the UK
* Moving their place of residence out of the UK
* Providing an invalid Date of Birth (DoB)
* Providing an invalid National Insurance (NI) number

**When customer moves their address out of the UK, they can continue making subscriptions until the current tax year ends.**  
We are notified of address changes via the [**PUT /v1/customers/retail/{customerId}/currentaddress**](/uk/docs/embedded-banking/retail-customers#update-an-existing-retail-customers-address) endpoint.  
After you set the customer's address back to the UK, you must submit a new declaration of ISA eligibility before  the customer can pay into their ISA again.

**When a customer changes their place of residence out of the UK, no further subscriptions can be made.**  
We are notified of country of residence changes via the [**PATCH /v1/customers/retail/{customerId}**](/uk/docs/embedded-banking/retail-customers#update-an-existing-retail-customers-personal-information) endpoint.  
After you set the customer's place of residence back to the UK, you must submit a new declaration of ISA eligibility before the customer can pay into their ISA again.

**When a customer's DoB or NI number is found to be invalid, no further subscriptions can be made.**  
We receive these details via the [**PATCH /v1/customers/retail/{customerId}**](/uk/docs/embedded-banking/retail-customers#update-an-existing-retail-customers-personal-information) endpoint.  
After the details are corrected, you must submit a new declaration of ISA eligibility before the customer can pay into their ISA again.

You can submit a declaration of ISA eligibility using the [**POST /isas/{accountId}/declaration**](#set-a-new-isa-declaration) endpoint.
