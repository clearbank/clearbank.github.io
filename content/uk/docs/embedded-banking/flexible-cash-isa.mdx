...
- [Manual cash ISA transfer in](#manual-transfer-in)
- [Manual cash ISA transfer out](#manual-transfer-out)
- [Automated cash ISA transfer in](#automated-transfer-in)
- [Automated cash ISA transfer out](#automated-transfer-out)
...
## Manual cash ISA transfer in or out
...
**Timeline:** Governed by HMRC’s SLA of **15 working days**.
...
## Automated cash ISA transfer in or out

The Pay.UK Automated Cash ISA Transfer Service is a secure, digital system that enables customers to move their Cash ISA savings from one provider to another without paperwork, faster, and with greater transparency.

It replaces the traditional manual process—where forms are exchanged between banks—with a fully electronic workflow. 

<Callout colour="blue">
This is only available when both ISA providers are accredited participants of the Pay.UK scheme.
</Callout>

To begin a transfer in, the customer must request the transfer from the current institution holding the account (the ceding party).
...
<EndpointBlock
  ...
  endpoints={[
    {
      path: "/v1/institutions/{institutionId}/isas/{accountId}/automated-transfer-in",
      version: "1.0AutomatedTransfer",
    }
  ]}
/>
...
<EndpointBlock
  ...
  endpoints={[
    {
      path: "/v1/institutions/{institutionId}/isas/{accountId}/automated-transfer-in/{urn}",
      version: "1.0AutomatedTransfer",
    }
  ]}
/>
...
<EndpointBlock
  ...
  endpoints={[
    {
      path: "/v1/institutions/{institutionId}/isas/{accountId}/automated-transfer-out/{urn}",
      version: "1.0AutomatedTransfer",
    }
  ]}
/>