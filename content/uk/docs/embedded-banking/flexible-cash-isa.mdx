...
<Callout colour="blue">
<p>Automated transfers are only available when both ISA providers are accredited participants of the Pay.UK scheme. If one provider is not a participant, the existing <a href="https://clearbank.github.io/uk/docs/embedded-banking/flexible-cash-isa#manual-cash-isa-transfer-in-or-out">manual transfer process</a> must be used.</p>
</Callout>
...
An automated transfer in refers to a ceding party (the current holders of the ISA) requesting the transfer of an existing ISA to ClearBank via Pay.UK’s automated service.
...
5. The ceding party receives a **Transfer Request **, validate the details and respond with a **Transfer Acceptance message**
...
9. The ceding party sends the funds to a ClearBank transfer hub account
10. You receive the **Transfer Status Update webhook** with the `State` of `PaymentReceived`
11. You call the **POST /v3/payments/fps** endpoint to move the funds from the transfer hub account (created during onboarding) to the customers new account
...
8. You receive the **Transfer Status Update webhook** with the `State` of `ReadyForPayment`
9. You call the **POST /v3/payments/fps** endpoint to move the funds from the cash ISA to the transfer hub account (created during onboarding)
10. You receive the **Transaction Settled webhook**
11. You call the **POST /v3/payments/fps** endpoint to move the funds from the transfer hub account to the account details provided by the acquiring party
12. You receive the **Transaction Settled webhook**
...
If transfer is referred, a Transfer Referral message is sent to the acquiring party. Once referred:
...
**Rejected:**
- A fuzzy match check on the customer's given name, surname, address, date of birth, National Insurance number, and account details returns no match or too fuzzy
