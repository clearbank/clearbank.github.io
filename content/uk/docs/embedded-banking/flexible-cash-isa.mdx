...
Because cash ISAs are structured differently to other account types, you will need to use the endpoints defined on this page to interact with them. Do not use the endpoints listed on the [Manage sterling accounts page](/uk/docs/gbp-accounts/manage-accounts).
...
### Set a new declaration for an ISA

Under UK ISA rules, if a customer changes their place of residence to a non-UK address in a given tax-year, the subscription for their ISA remains valid until the end of that tax-year.  
However, if their stated place of residence is still non-UK in the following tax-year, then no further subscriptions are allowed.

A new declaration of eligibility is required for such customers. To redeclare a customer's eligibility once they move back to the UK, you can use the [**POST /isas/{accountId}/declaration**](#set-a-new-isa-declaration) endpoint.

ISA ineligibility can also be caused by an invalid National Insurance number or date of birth. You can redeclare eligibility for these with the same [**POST /isas/{accountId}/declaration**](#set-a-new-isa-declaration) endpoint too.
