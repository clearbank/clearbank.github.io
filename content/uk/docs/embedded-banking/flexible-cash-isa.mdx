...
A cash ISA (Individual Savings Account) is a real savings deposit account held with ClearBank that is subject to certain tax benefits <ExternalLink href="https://www.gov.uk/individual-savings-accounts">defined by the UK government</ExternalLink>. It is eligible for [FSCS protection](/uk/docs/api/glossary#financial-services-compensation-scheme), and can be used to send or receive payments once a [nominated account](#nominated-accounts) has been defined.

A [flexible cash ISA](/uk/docs/api/glossary#flexible-cash-isa) lets you withdraw and re-deposit funds in to your account, in the same tax-year, without affecting your annual ISA allowance.  

<Callout colour="blue">
  Deposits held in ClearBank cash ISAs may be eligible for FSCS protection. FSCS protection insures deposits of up to £120,000 per person or legal entity per authorised firm (ClearBank) in the event of insolvency. Customers should review the specific terms and conditions of their account to confirm whether FSCS protection applies.
</Callout>

### Nominated accounts

A **nominated account** is the account that you transfer funds from. Upon closure of a cash ISA, remaining funds and interest are transferred into it.

The balance of a cash ISA is held by ClearBank and can only be viewed using the ClearBank API. Cash ISAs are reconciled for you by ClearBank. 

To open a cash ISA, your customer must provide you with their national insurance (NI) number and confirm that they are a resident of the UK.

<Callout colour="blue">
Funds can be transferred from either the nominated account or another ClearBank account held by your organisation. Name matching between ClearBank accounts is <strong>your</strong> responsibility.
</Callout>

As cash ISAs are structured differently to other GBP account types, you must use the endpoints defined here to interact with them. Do **not** use the endpoints described in [Manage sterling accounts](/uk/docs/gbp-accounts/manage-accounts).

You can create and manage customers with cash ISAs using the endpoints listed in [Retail customers](/uk/docs/embedded-banking/retail-customers).

The following sections explain how to:
- [Set up a cash ISA](#set-up-a-cash-isa)
- [Close or cancel a cash ISA](#end-of-life-for-a-cash-isa)
- [Manually transfer in a cash ISA](#transfer-a-cash-isa-in-manually)
- [Manually transfer out a cash ISA ](#transfer-a-cash-isa-out-manually)
- [Automatically transfer in a cash ISA ](#transfer-a-cash-isa-in-automatically)
- [Automatically transfer out cash ISA ](#automated-cash-isa-transfer-out)

### Set up a cash ISA

To create a new cash ISA, you need to create an account, then set up the nominated account.

<Callout colour="blue">
If the customer does not already exist, create a retail customer first using the <a href="../embedded-banking/retail-customers#create-a-retail-customer">POST /v1/customers/retail</a> endpoint.
</Callout>
...
![Submit the POST /v1/isas endpoint. Once you receive a 201 response, submit the PUT /v1/accounts/{accountId}/nominated-account to set a nominated account. Once you receive a 200 response, funds can now be added to the cash ISA.](/assets/images/diagrams/uk/cash-isa/cash-isa-open.svg)

At this point, the account is open and fully operational. 

You can retrieve information on all ISAs using the [GET /v3/accounts](#get-all-accounts-real) endpoint and filtering for ISAs. You can also retrieve details on a specific cash ISA using the [GET /v1/isas/{accountId}](#get-cash-isa-details) endpoint.

To amend the details or status of a cash ISA, use the [PATCH /v1/accounts/{accountId}](#amend-cash-isa-details-or-status) endpoint.
...
There are several ways to close a cash ISA. Choose the appropriate method based on what has happened in the account journey and what the customer wants to do with the account next.

| Action                        | What this does                                                                                                                                                  |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Close](#close)               | Closes the account.                                                                                                                                             |
| [Cancel](#cancel)             | Cancels and closes the account without registering the account with the UK government. The account must be less than 14 days old and must have had funds deposited. |
...
Use the following diagram to validate if the account should be closed, cancelled or transferred out:

<img src="/assets/images/diagrams/uk/cash-isa/cash-isa-life-cycle.svg" alt="A decision tree diagram explaining the life cycle of a cash ISA." width="100%"/>
...
#### Close
When closing a cash ISA, you must withdraw funds before closing the account.

<Callout colour="blue">
If the cash ISA is under 14 days old and has had money deposited, use the <a href="#cancel-a-cash-isa">cancel a cash ISA</a> endpoint.
</Callout>
...
If the account balance is not zero, the request will be rejected. 

<Callout colour="red">⚠️ <strong>Account closure requests cannot be undone</strong>.</Callout>
...
Once the account closure process is complete, you will receive either the `Account Closure Completed` or `Account Closure Failed` webhook.

If the account closure fails, re-request the closure.

The process is shown in this message flow diagram:

![Submit the POST /v1/isas/{accountId}/cancellation endpoint. Once you receive a 202 response. As the process is asynchronous, you can check on the closure status using the GET /v1/accounts/{accountId}/closure endpoint. Once the closure is confirmed, you will receive either the Account Closure Completed or Account Closure Failed webhook.](/assets/images/diagrams/uk/cash-isa/cash-isa-close.svg)

#### Cancel
When cancelling a cash ISA, the account must be less than 14 days old. This endpoint also closes the account.
...
Money must have already been deposited into the cash ISA. If no money has been deposited, or the account is over 14 days old, use the <a href="#close-a-cash-isa">POST /v1/accounts/{accountId}/closure</a> endpoint.
...
Once the cancellation process is complete, you will receive either the `Account Closure Completed` or `Account Closure Failed` webhook.

The process is shown in this message flow diagram:

![Message flow diagram of the cancellation process.](/assets/images/diagrams/uk/cash-isa/cash-isa-cancel.svg)
...
Ineligible customers can still withdraw funds from their cash ISA.

Customers can be made ineligible for cash ISA subscriptions for these reasons:

* You notify us that the customer's address is now outside the UK.
* You notify us that the customer's place of residence is now outside the UK.
* You provide us with an invalid date of birth for the customer.
* You provide us with an invalid National Insurance (NI) number for the customer.

**When a customer moves their address out of the UK, they can continue making subscriptions until the current tax year ends.**  
You should notify us of customer address changes using the [**PUT /v1/customers/retail/{customerId}/currentaddress**](/uk/docs/embedded-banking/retail-customers#update-an-existing-retail-customers-address) endpoint.  
...
You should notify us of customer country of residence changes using the [**PATCH /v1/customers/retail/{customerId}**](/uk/docs/embedded-banking/retail-customers#update-an-existing-retail-customers-personal-information) endpoint.  
...
You can use the [Faster Payments](/uk/docs/api/glossary#faster-payments) endpoint to subscribe to or withdraw from a cash ISA.
...
**Manual transfer:** Transfers are initiated and managed manually through ClearBank’s API and email workflows. The process involves coordination between the new and old ISA providers using forms like the Transfer Authority Form (TAF) and Transfer History Form (THF). <br/>
**Timeline:** Governed by HMRC’s SLA (Service Level Agreement) of **15 working days**.

**Automated transfer:** Transfers are initiated and tracked through ClearBank’s API endpoints integrated with Pay.UK’s automated service. This includes `POST`, `PUT`, and `PATCH` endpoints for initiating, resubmitting, and updating transfer requests. <br/>
...
![Message flow diagram of the transfer in process, as detailed.](/assets/images/diagrams/uk/cash-isa/cash-isa-transfer-in.svg)

<EndpointBlock
  type="post"
  title="Transfer a cash ISA in manually"
  endpoints={[
    {
...
![Message flow diagram of the transfer out path, as detailed.](/assets/images/diagrams/uk/cash-isa/cash-isa-transfer-out.svg)

<EndpointBlock
  type="post"
  title="Transfer a cash ISA out manually"
  endpoints={[
    {
...
An automated transfer in happens when a customer approaches you to transfer their ISA from another ISA Manager (the ceding party) to ClearBank using Pay.UK's ISA transfer service.

To complete an automated transfer in: 
1. The customer makes a request that you to transfer the cash ISA from the current institution holding the account (the ceding party).
2. Call the **POST /v1/isas/{accountId}/automated-transfer-in** endpoint providing the necessary details.
...
![Message flow diagram of the automated transfer in process, as detailed.](/assets/images/diagrams/uk/cash-isa/tisa-transfer-in-success-message-flow.svg)

<EndpointBlock
  type="post"
  title="Transfer a cash ISA in automatically"
  endpoints={[
    {
...
<EndpointBlock
  type="put"
  title="Resubmit an automated cash ISA transfer in"
  endpoints={[
    {
...
![Message flow diagram of the automated transfer in process where the transfer is referred. This follows the automated transfer in steps with an additional response from the ceding party advising of the referral. This in turn triggers a status update webhook with a state of Referred.](/assets/images/diagrams/uk/cash-isa/tisa-transfer-in-referred-accepted-message-flow.svg)

**or reject the transfer:**

 ![Message flow diagram of the automated transfer in process where the transfer is referred. This follows the automated transfer in steps with an additional response from the ceding party advising of the referral. This in turn triggers a status update webhook with a state of Referred. The ceding party will then advise of the rejection through the Transfer Rejected Message which triggers another status update webhook with a state of Rejected.](/assets/images/diagrams/uk/cash-isa/tisa-transfer-in-referred-rejected-message-flow.svg)

If the transfer is rejected, the customer should decide how they wish to proceed, either:

- **Resubmit the transfer request**: Call the [**PUT /v1/isas/{accountId}/automated-transfer-in/{urn}**](#resubmit-an-automated-cash-isa-transfer-in) and provide the URN of the existing transfer.
- **Abandon the transfer request**: Update the existing transfer request to abandoned by calling the **PATCH /v1/isas/{accountId}/automated-transfer-in/{urn}**, providing the URN of the existing transfer, and updating the `State` to `Abandoned`.
...
![Message flow diagram of the automated transfer out process.](/assets/images/diagrams/uk/cash-isa/tisa-transfer-out-success-message-flow.svg)
...
## Simulating ISA transfer scenarios

As part of your simulation onboarding process, you are required to simulate several scenarios to prove your system's capability before graduating to the production environment.

<EndpointBlock
  type="post"
  title="Simulate an automated outbound transfer"
  endpoints={[
    {
      path: "/v1/isas/automated-transfers/injector/transfer-out",
      version: "1.0.isaMIT",
      webhooks: [
      ]
    }
...
<EndpointBlock
  type="post"
  title="Update the state of a simulated transfer"
  endpoints={[
    {
      path: "/v1/isas/automated-transfers/injector/{urn}/update-state",
      version: "1.0.isaMIT",
      webhooks: [
      ]
    }
...
<EndpointBlock
  type="post"
  title="Simulate acceptance of an automated transfer"
  endpoints={[
    {
      path: "/v1/isas/automated-transfers/injector/{urn}/accept",
      version: "1.0.isaMIT",
      webhooks: [
      ]
    }
