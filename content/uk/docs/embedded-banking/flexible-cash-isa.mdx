...
If a customer moves address out of the UK, their ISA subscription will be invalidated at the start of the next tax-year. They will not be able to pay into their ISA. After their address returns to the UK, a new declaration of ISA eligibility must be provided before they can pay into their ISA again.

You can redeclare a customer's eligibility once they move back to the UK using the [**POST /isas/{accountId}/declaration**](#set-a-new-isa-declaration) endpoint.

ISA ineligibility can also be caused by a providing the wrong National Insurance number or date of birth. When this information is corrected, you can redeclare their eligibility for these with the same [**POST /isas/{accountId}/declaration**](#set-a-new-isa-declaration) endpoint.
