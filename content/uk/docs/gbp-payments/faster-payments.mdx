...
The [Faster Payment System (FPS)](/uk/docs/api/glossary#faster-payments) is the most frequently used and fastest payment rail in the UK. It is instant or near-instant, and operates all hours of all days. All ClearBank UK GBP accounts support inbound and outbound Faster Payments.
...
In line with legislation to prevent [Authorised Push Payment (APP) scams](/uk/docs/api/glossary#app-scam), payments sent using the Faster Payments scheme may be in scope for APP scam reimbursement, allowing for the recovery of 50% of any consumer reimbursement (up to the claim limit) from the receiving [Payment Service Provider (PSP)](/uk/docs/api/glossary#payment-service-provider). Further details are available on <ExternalLink href="https://www.wearepay.uk/appr-policy/">Pay.UK's website</ExternalLink>.

Only payments sent through scheme are in scope of APP scam reimbursement, however, when you initiate a payment and we identify that the recipient account is held by a ClearBank client, the payment method is updated to internal transfer by default.
...
![Message flow diagram showing a single or bulk SOP payment sent outside of supported hours and returning a 400 Bad Request response](/assets/images/diagrams/uk/faster-payments/sop-payment.svg "SOP sent outside of supported hours")
...
![Message flow diagram showing abulk payment containing SIPs, SOPs and FDPs sent outside of supported hours, a 202 Accepted response is returned, the SIPs and FDPs are accepted for processing and the SOPs are rejected](/assets/images/diagrams/uk/faster-payments/bulk-sop-payment.svg "Bulk payment containing SIPs, SOPs and FDPs sent outside of supported hours")
...
ClearBank does not permit the return of Faster Payments over **90 calendar days old** or payments that are **temporarily ineligible for return** (for example, if the transaction has not yet settled or been reflected as settled throughout our system). If a return cannot be completed (including after retry attempts), you'll receive a **'400 - Bad Request'** response.

To return a payment older than 90 days, or a payment you cannot send through the return endpoint, make a normal outbound payment instead.
...
#### Faster Payments Transaction Settled webhook error codes

| Error code                       | Description                                                                                | Recommended action                                                   | Retry Policy      |
|----------------------------------|--------------------------------------------------------------------------------------------|----------------------------------------------------------------------|-------------------|
| **AC03**                             | Beneficiary Sort Code/Account Number unknown                                               | Verify beneficiary account details and resubmit                      | Review then retry |
| **AC06**                             | Beneficiary Account stopped                                                                | Contact beneficiary to confirm account status or provide alternative | Review then retry |
| **AC07**                             | Beneficiary Account closed                                                                 | Contact beneficiary for updated account details                      | Review then retry |
| **AC09**                             | Account is not in currency quoted                                                          | Confirm currency requirements and resend in supported currency       | Review then retry |
| **AG01**                             | Terms and Conditions of Account do not permit crediting of these funds                     | Contact beneficiary for alternative payment method or account        | Do not retry      |
| **BE08**                             | Error stems from participant error or unavailability                                       | Retry payment; if it continues to fail, wait and then retry   | Retry             |
| **CB_AccountHoldersBankrupt**        | Payment cannot be applied because of Beneficiary sensitivities – for example, possible bankruptcy  | Do not retry; escalate and seek legal/compliance guidance            | Do not retry      |
| **CB_AccountNameAndNumberMismatch**  | The payment has been rejected by the receiving bank because the beneficiary account name does not match the beneficiary account number | Validate account name against account number (for example, using CoP) before retrying | Review then retry |
| **CB_AccountSwitched**               | Account transferred                                                                        | Use account switching data (for example, CASS redirection) or request updated details. Follow account switching/redirection guidance or obtain updated details  | Do not retry |
| **CB_InitiatingPartyActionRequired** | Sending FPS Institution action required                                                    | Review payment details and format and retry. If payment fails again, contact ClearBank | Review then retry |
| **CB_PaymentBeneficiaryNameMissing** | The payment has been rejected by the receiving bank as the beneficiary name is not present | Include beneficiary name and retry                                   | Review then retry |
| **CB_PaymentReferenceIncorrect**     | The payment has been rejected by the receiving bank due to reference information field being incorrect | Correct the reference format/content and resubmit        | Review then retry |
| **CB_PaymentReferenceMissing**       | The payment has been rejected by the receiving bank because the receiving account cannot be identified without a Payee Reference | Ensure mandatory payment reference is included and retry | Review then retry |
| **MD09** | Beneficiary deceased | Do not retry; contact beneficiary organisation or sender for next steps | Do not retry |
| **MS03** | Reasons not specified by end agent | Retry or contact beneficiary bank for clarification | Retry |
| **PY01** | The payment has been rejected due to an unknown sort code/account number | Reconfirm sort code and account number before resubmission | Review then retry |
