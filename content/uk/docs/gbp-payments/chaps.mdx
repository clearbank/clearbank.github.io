...
<p>These endpoints will remain available for use until <strong>13 November 2026</strong>. Version 6 of these endpoints is now available. If you have any questions, reach out to your Client Director.</p> <p>For further information, refer to our <a href="../api/versioning">versioning policy</a> and <a href="../api/support-life-cycle">support lifecycle information</a>.</p></Callout>

The [Clearing House Automated Payment System (CHAPS)](/uk/docs/api/glossary#chaps) is the UK's high-value payment system. ClearBank accounts support inbound and outbound CHAPS payments, without an associated amount limit.

You can send or return CHAPS payments between 08:00 – 17:00 Monday to Friday (excluding UK public holidays).

- CHAPS payments can be sent using the [POST /payments/chaps/v6/customer-payments](#send-a-chaps-payment) endpoint.
- Settlement details are provided to you through the [Transaction Settled](#transaction-settled-webhook) webhook. 
- If you need to return a CHAPS payment, you can do so using the [POST /payments/chaps/v6/return-payments](#return-a-chaps-payment) endpoint.

<Callout colour="blue">
  It is important to verify your firewall settings so that they do not block <a href="#transaction-settled-webhook">Transaction Settled webhook</a> notifications, which can include a large payload due to the size of the Iso20022XmlDocument field.
</Callout>
...
The version 6 endpoints introduce the changes required to remove unstructured address types, together with a small number of ClearBank enhancements. If you are migrating from version 5, the key differences are:

**Customer payments (pacs.008) — `POST /payments/chaps/v6/customer-payments`**
- The Creditor's `postalAddress` is now **mandatory** (optional in v5). This enriches payment information with additional creditor details to reduce follow-on queries, improve STP processing, and future-proof you for upcoming scheme-mandated requirements.
- On the `Other` account identification block for `sourceAccount`, `debtorAccount` and `creditorAccount`: `schemeName` is now enforced and the legacy `proprietary` field has been removed.
- The amount field has been renamed from `instructedAmount` to `interBankSettlementAmount` to more accurately align with ISO 20022 wording.

**Institution / bank-to-bank payments (pacs.009) — `POST /payments/chaps/v6/institution-payments`**
- On the `Other` account identification block for `sourceAccount` and `creditorAccount`: `schemeName` is now enforced and the legacy `proprietary` field has been removed.
- The amount field has been renamed from `instructedAmount` to `interBankSettlementAmount`.

<Callout colour="blue">
For BoE funding clients: the new v6 and current v5 endpoints, as well as all Institution Portal screens, remain available until <strong>17:55</strong> UK time. We recommend that clients who complete Bank of England account funding through the ClearBank Portal screens integrate with the pacs.009 v6 endpoint to offer an alternative means of submitting these payments.
</Callout>

**Return payments (pacs.004) — `POST /payments/chaps/v6/return-payments`**
- A new optional `additionalInformation` field (105 characters) has been added to allow reason context to be supplied with a return. This field is **required** when the return reason code is `NARR`.

### Hybrid address

From 24 November 2025, some inbound CHAPS payments may include a new address format called hybrid address. A hybrid address is an address format that includes both structured fields (such as town name and country) and unstructured address lines.

The structured and unstructured address formats will also continue to be used alongside the hybrid address format. These formats appear in the `Iso20022XmlDocument` field in the [Transaction Settled webhook](#transaction-settled-webhook).

Hybrid address is being introduced as part of the Bank of England’s CHAPS updates to harmonise payment standards across jurisdictions in line with the Cross Border Payment Reporting Plus (CBPR+) requirements.

Each format has different requirements:
- Hybrid address formats contain up to two `<AdrLine>` XML tags of up to 70 characters each, alongside mandatory `<TwnNm>` and `<Ctry>` address tags
> Information received in the `<AdrLine>` XML tags should be distinct from what is received in the structured address elements. If a payment message contains duplicate information across structured and unstructured fields, we expect it will be rejected by the Bank of England.
- Unstructured address formats contain up to three `<AdrLine>` XML tags of up to 35 characters each

You can extract all address information from `<AdrLine>` XML tags, regardless of whether you receive a hybrid or unstructured address.

<Callout colour="blue">
In November 2026, the Bank of England will fully retire unstructured address formats.
</Callout>

Ensure your systems can process hybrid address formats for inbound payments. No changes are required for outbound payments as they already use structured address.
...
In line with the Bank of England's [ISO 20022](/uk/docs/api/technical-reference#iso-20022) data enhancement requirements, you will be required to provide additional details on the debtor and creditor such as the Legal Entity Identifier (LEI), Purpose Code and Category Purpose Code.
...
The [LEI](/uk/docs/api/glossary#legal-entity-identifier) is a 20-character, alpha-numeric code based on the [ISO 17442]() standard and contains information about an entity’s ownership structure. Providing the LEI is mandatory for [pacs.008](/uk/docs/api/technical-reference#pacs-008) where the party being described (debtor or creditor) is a financial institution. Otherwise, it is an optional field but strongly recommended to include when the party is an organisation.
...
To help make sense of the various codes, they have been broken down into eight categories in the following table. The categories have been created by ClearBank to support our clients in finding the appropriate codes for their use case when making GBP [Cross-Border payments](/uk/docs/api/glossary#cross-border-payment).
...
The Creditor field in CHAPS pacs.008 messages identifies the party receiving the funds. This party may be an **individual**, a **business**, or a **financial institution**. The correct population of this field is essential for accurate payment routing and compliance with ISO 20022 standards.

<Callout colour="blue">
The <strong>CreditorAgent</strong> field is not required in the CHAPS pacs.008 endpoint, as this information can be inferred from the UK sort code.
</Callout>

Depending on the creditor type, you will need to populate either the `PrivateIdentifier` or `OrganisationIdentifier` block within the `Creditor` object:

|Creditor type	|Creditor object population	| Creditor name field population |
|---------------|---------------------------|--------------------------------|
|Individual	|PrivateIdentifier	| Full name of the individual |
...
**Note:** This only applies to payments initiated by you in the Bank of England system and sent to your ClearBank account (defunding scenario). There is no impact to outbound CHAPS pacs.009 payments sent from ClearBank.
...
<Callout colour="blue">
The availability of the <strong>POST /payments/chaps/v6/institution-payments</strong> endpoint has been extended until <strong>17:55</strong> UK time. This aligns with the Bank of England funding screen currently provided to clients in the ClearBank Portal.
</Callout>
...
<p>These endpoints will remain available for use until <strong>13 November 2026</strong>. Reach out to your Client Director with any questions.</p>
...
To support Bank of England changes effective 13 November 2026, version 3 of the simulator endpoints for inbound CHAPS payments is now available. Version 3 removes support for unstructured address types, as well as other Bank of England mandated changes. This aligns with international [SWIFT](/uk/docs/api/glossary#swift) standards.

<Callout colour="blue">
    Outbound CHAPS payments sent through ClearBank already use structured address formatting. No changes are required to your outbound integration.
</Callout>

Update your test integration to ensure continued compatibility with the hybrid address format.

- Simulate an inbound CHAPS payment (pacs.008) through the [POST /inbound-payment-simulation/chaps/v3/customer-payments](#simulate-an-inbound-chaps-payment) endpoint.
- Simulate a returned CHAPS payment (pacs.004) through the [POST /inbound-payment-simulation/chaps/v3/return-payments](#simulate-a-returned-chaps-payment) endpoint.
- Simulate a CHAPS bank-to-bank payment (pacs.009) through the [POST /inbound-payment-simulation/chaps/v3/institution-payments](#simulate-a-chaps-bank-to-bank-payment) endpoint.

<Callout colour="blue">
    The <a href="../gbp-payments/chaps#simulate-a-chaps-return-payment-using-a-simplified-request">POST /v2/inbound-payment-simulation/return-payment-simplified</a> endpoint has been deprecated. All requests through the new v3 <a href="#simulate-a-returned-chaps-payment">POST /inbound-payment-simulation/chaps/v3/return-payments</a> use a simplified request format.
</Callout>

<Callout colour="blue">
You should use the v3 API inbound simulator endpoints to simulate inbound CHAPS payments. Do not use the legacy web-based URL simulator.
</Callout>

#### What's changed in simulator version 3
If you are migrating from the version 2 simulator endpoints, the key differences are:

**Customer payments (pacs.008)**
- The amount field has been renamed from `instructedAmount` to `interBankSettlementAmount`.
- The Debtor `addressLine1` and `addressLine2` fields have been removed, as SWIFT's intention is to no longer support unstructured address formats.
- The Creditor `addressLine1` and `addressLine2` fields have been removed, as SWIFT's intention is to no longer support unstructured address formats.
- The `instructingAgentBIC` is now intelligently deduced from the Debtor details provided; a standard default value is used if not provided.
- If you supply an `instructingAgentBIC`, it is validated to ensure it is a valid CHAPS Direct Participant BIC.

**Institution payments (pacs.009)**
- The amount field has been renamed from `instructedAmount` to `interBankSettlementAmount`.
- The Debtor `addressLine1` and `addressLine2` fields have been removed, as SWIFT's intention is to no longer support unstructured address formats.
- The Creditor `addressLine1` and `addressLine2` fields have been removed, as SWIFT's intention is to no longer support unstructured address formats.
- The `instructingAgentBIC` is now intelligently deduced from the Debtor details provided; a standard default value is used if not provided.
- If you supply an `instructingAgentBIC`, this is validated to ensure it is a valid CHAPS Direct Participant BIC.

**Return payments (pacs.004)**
- The amount field has been renamed from `instructedAmount` to `returnedInterBankSettlementAmount`.
- A new optional `additionalInformation` field (105 characters) has been added. The `NARR` return reason code is supported when this field is populated.
- The `returnReasonCode` field has been renamed to `reason` to align with the outbound pacs.004 API endpoint.
- The `paymentId` in the request must reference an outbound CHAPS payment previously initiated in the simulation environment.
...
<EndpointBlock
    ...
    endpoints={[
    {
        path: "/inbound-payment-simulation/chaps/v3/customer-payments",
        version: "3.0-ChapsSim",
        webhooks: [
        'transaction-settled-webhook-v6',
        ]
    },
...
**Payload for a structured address**

```
{
  "InstructionIdentification": "12345678TEST1234567899100",
  "EndToEndIdentification": "12345678TEST12345678100",
  "InstructedAmount": {
    "Amount": 1001,
    "Currency": "GBP"
  },
  "InstructingAgentBic": "LOYDGB22XXX",
  "DebtorAccount": {
    "Iban": "GB71CPBK08929969687998"
  },
  "Debtor": {
    "Name": "Name 1",
    "PostalAddress": {
      "BuildingNumber": "No 12",
      "BuildingName": "LL House",
      "StreetName": "StreetName Value 5",
      "TownName": "TownName Value 6",
      "Country": "GB",
      "PostCode": "NN4 5QQ"
    },
  ...
  "CreditorAccount": {
    "Iban": "GB86CLRB04056810005205"
  },
  "Creditor": {
    "Name": "Name 2",
    "PostalAddress": {
      "BuildingNumber": "No 27",
      "BuildingName": "AA House",
      "StreetName": "StreetName Value 20",
      "TownName": "TownName Value 21",
      "Country": "GB",
      "PostCode": "CC9 0FF"
    },
    "OrganisationIdentifier": null,
...
**Payload for a hybrid address**
```
{
  "InstructionIdentification": "12345678TEST1234567810011",
  "EndToEndIdentification": "12345678TEST1234567810011",
  "InstructedAmount": {
    "Amount": 1001,
    "Currency": "GBP"
  },
  ...
  "Debtor": {
    ...
    "PostalAddress": {
      ...
      "Country": "GB",
      "PostCode": "NN4 5QQ",
      "AddressLine1": "57 Hello World Street, London, EC3A 1AB",
      "AddressLine2": "57 Hello World Street, London, EC3A 1AB"
    },
    "OrganisationIdentifier": null,
    "PrivateIdentifier": null
  },
...
<EndpointBlock
    ...
    endpoints={[
    {
        path: "/inbound-payment-simulation/chaps/v3/return-payments",
        version: "3.0-ChapsSim",
        webhooks: [
        'transaction-settled-webhook-v6',
        ]
    ...
    {
        path: "/inbound-payment-simulation/chaps/v2/return-payments",
        version: "2.0-ChapsSim",
        webhooks: [
        'transaction-settled-webhook-v6',
        ]
...
<EndpointBlock
    type="post"
    title="Simulate a CHAPS return payment using a simplified request"
    endpoints={[
    {
        path: "/v2/inbound-payment-simulation/return-payment-simplified",
        version: "2.0-ChapsSim",
        webhooks: [
        'transaction-settled-webhook-v6',
        ]
...
<EndpointBlock
    ...
    endpoints={[
        {
        path: "/inbound-payment-simulation/chaps/v3/institution-payments",
        version: "3.0-ChapsSim",
        webhooks: [
        'transaction-settled-webhook-v6',
        ]
...
| G004       | CreditPendingFunds                       | In a FIToFI Customer Credit Transfer: Credit to the creditor’s account is pending, status Originator is waiting for funds provided by a cover. Update will follow from the Status Originator. |
...
| SVNR       | ServiceNotRendered                       | The card payment is returned since a cash amount rendered was not correct or goods or a service was not rendered to the customer, for example, in an e-commerce situation.     |
