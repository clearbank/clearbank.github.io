...
The version 6 endpoints introduce the changes required by the Bank of England's November 2026 CHAPS release, together with a small number of ClearBank enhancements. If you are migrating from version 5, the key differences are:
...
- The amount field has been renamed from `instructedAmount` to `interBankSettlementAmount`.
- The Debtor `addressLine1` and `addressLine2` fields have been removed, as SWIFT no longer supports unstructured address formats.
- The Creditor `addressLine1` and `addressLine2` fields have been removed, as SWIFT no longer supports unstructured address formats.
