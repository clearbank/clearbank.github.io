...
<Callout colour="blue">
The **CreditorAgent** field is not required in the CHAPS pacs.008 endpoint, as this information can be inferred from the UK sort code.
</Callout>

|Creditor type	|Creditor field population	|
|---------------|---------------------------|
|Individual	|Full name of the individual	|
|Business	|Registered business name	|
|Financial institution	|Institution name (if acting as creditor)	|

#### Example scenarios

1. **Payment to an individual**  
   - Creditor: `Emily Clarke`  
   - Sort Code: `10-00-01` (example UK sort code)

2. **Payment to a business**  
   - Creditor: `Greenfield Engineering Ltd`  
   - Sort Code: `20-45-67` (example UK sort code)

3. **Payment to a financial institution**  
   - Creditor: `Example Savings Bank`  
   - Sort Code: `30-99-88` (example UK sort code)

In all cases, the sort code provided in the payment message enables identification of the financial institution holding the account, removing the need for a separate Creditor Agent field.
