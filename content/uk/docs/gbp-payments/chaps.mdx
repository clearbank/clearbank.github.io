...
The version 6 endpoints introduce the changes required to remove unstructured address, together with a small number of ClearBank enhancements. If you are migrating from version 5, the key differences are:

**Customer payments (pacs.008) — `POST /payments/chaps/v6/customer-payments`**
- The Creditor's `postalAddress` is now **mandatory** (optional in v5), in line with the BoE Name-Address scheme mandate.
- The Legal Entity Identifier (LEI), Purpose Code and Category Purpose Code are now **mandatory** on the Debtor and Creditor (see [Version 6 mandatory fields](#version-6-mandatory-fields)).
...
- On the `Other` party identification block for the Debtor and Creditor: `schemeName` is now enforced and the legacy `proprietary` field has been removed.
- The amount field has been renamed from `instructedAmount` to `interBankSettlementAmount` to align with ISO 20022 wording.
...
- A new optional `additionalInformation` field has been added to allow reason context to be supplied with a return. This field is **required** when the return reason code is `NARR`.
