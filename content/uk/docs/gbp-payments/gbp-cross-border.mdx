...
Version 4 of the GBP Cross-Border endpoint introduces the changes required to remove unstructured address, together with a small number of ClearBank enhancements. If you are migrating from version 3, the key differences are:
...
- Additional charge bearer options are now supported beyond `SHAR`, giving you more flexibility over how charges are allocated between the debtor and creditor.
- `Other` remains a permitted value for the Creditor Account scheme name to accommodate non-UK identification formats — this is a difference from the domestic CHAPS v6 endpoints, where `Other` is not supported.
...
2. **Creditor Agent**: The financial institution that holds the account into which the funds are credited. This is always a financial institution and must be identified using a valid 11-character or 8-character **Business Identifier Code (BIC)**.
