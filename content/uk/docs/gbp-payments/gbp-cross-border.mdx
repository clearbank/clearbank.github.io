...
<p>This endpoint will remain available for use until <strong>13 November 2026</strong>. Version 4 of this endpoint is now available. Reach out to your Client Director with any questions.</p> <p>You can refer to our <a href="../api/versioning">versioning policy</a> and <a href="../api/support-life-cycle">support lifecycle information</a>.</p></Callout>
...
You can send GBP cross-border payments using the [POST /payments/cross-border-sterling/v4/payments](#send-a-gbp-cross-border-payment) endpoint. 

Settlement details are provided to you by the [Transaction Settled](#transaction-settled-webhook) webhook. 
...
The GBP Cross-Border endpoint supports the new [ISO 20022](/uk/docs/api/technical-reference#iso-20022) payment message formats, specifically [pacs.008](/uk/docs/api/technical-reference#pacs-008).

### What's changed in version 4

Version 4 of the GBP Cross-Border endpoint introduces the changes required to remove unstructured address types, together with a small number of ClearBank enhancements. If you are migrating from version 3, the key differences are:
...
- For the Creditor Account, GBP Cross-Border v4 offers `BBAN` and `Other` options in `schemeName`, whereas domestic CHAPS v6 offers `BBAN` and `SortcodeAccountNumber` options. (In both cases, these are alternatives to choosing the `IBAN` value.) The `Other` option in GBP Cross-Border allows you to accommodate non-UK identification formats required for cross-border payments.
...
- the [**remitter** (`Debtor`)](/uk/docs/api/glossary#debtor)
- the [**beneficiary** (`Creditor`)](/uk/docs/api/glossary#beneficiary)
...
The LEI is a 20-character, alpha-numeric code based on the [ISO 17442](/uk/docs/api/technical-reference#iso-17442) standard and contains information about an entity’s ownership structure. Providing the LEI is mandatory for [pacs.008](/uk/docs/api/technical-reference#pacs-008) where the party being described (debtor or creditor) is a financial institution. Otherwise, it is an optional field but strongly recommended to include when the party is an organisation.
...
The `Creditor` field in [POST /payments/cross-border-sterling/v4/payments](#send-a-gbp-cross-border-payment) (pacs.008 messages) identifies the party who will receive the funds. This party can be an **individual**, a **business**, or a **financial institution**.
...
In a payment transaction, there are two key parties:

1. **Creditor**: The person or organisation receiving the funds. This could be an individual, a business, or a financial institution (FI).
2. **Creditor Agent**: The financial institution that holds the account into which the funds are credited. This is always a financial institution and must be identified using a valid 11-character or 8-character **BIC**.

To ensure correct population of fields in pacs.008 payment messages, follow these guidelines based on the type of Creditor:

##### If the Creditor is an individual:
- Use the `privateIdentifier` object for the Creditor identification block.
- Do **not** populate the `Creditor > OrganisationIdentifier` object.
- **Populate** the `CreditorAgent > BIC` field with the BIC of the FI that holds the Creditor’s account.

##### If the Creditor is a business:
- Use the `organisationIdentifier` object for the Creditor identification block.
- Do **not** populate the `Creditor > PrivateIdentifier` object.
- **Populate** the `CreditorAgent > BIC` field with the BIC of the FI that holds the Creditor’s account.

##### If the Creditor is a financial institution:
- Use the `organisationIdentifier` object for the Creditor identification block.
- Do **not** populate the `Creditor > PrivateIdentifier` object.
- Optionally **populate** the `Creditor > OrganisationIdentifier > BIC` field with details of the FI's BIC.
- **Populate** the `CreditorAgent > BIC` field with the BIC of the FI that holds the Creditor’s account.
...
```
{
  ...
  "sourceAccount": {
    "iban": "GB00FAKE00000000000000"
  },
  "debtorAccount": {
    "iban": "GB00FAKE11111111111111"
  },
  "debtor": {
    "name": "Jane Smith",
    "postalAddress": {
...
**Sending a payment to a business**
```
{
  "instructionIdentification": "INST202509190002",
  "endToEndIdentification": "202509190654321",
  "instructedAmount": {
    "amount": 2500.00,
    "currency": "GBP"
  },
  ...
  "creditor": {
    ...
    "organisationIdentifier": {
      ...
      "otherIdentification": "2132334343434"
    }
  },
  "creditorAgent": {
    "bic": "CITIDEFFXXX"
  ...
  "categoryPurpose": "SALA",
  "remittanceInformation": {
    "creditorReferenceInformation": "INVOICE20250919"
  }
}
```

**Sending a payment to a financial institution**
```
{
  "instructionIdentification": "INST202509190003",
  "endToEndIdentification": "202509190987654",
  "instructedAmount": {
    "amount": 50000.00,
    "currency": "GBP"
  },
  "sourceAccount": {
    "iban": "GB00FAKE00000000000002"
  },
  "debtorAccount": {
    "iban": "GB00FAKE11111111111113"
  },
  "debtor": {
    "name": "Jane Smith",
    "postalAddress": {
  ...
  "remittanceInformation": {
    "creditorReferenceInformation": "TREASURY20250919"
  }
}
```
