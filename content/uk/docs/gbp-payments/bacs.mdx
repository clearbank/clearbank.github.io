...
When integrating with Bacs, you operate as either an **Originator (Often called Service User)** or a **Clearing PSP**, depending upon your role within the payment flow. These roles define how transactions are initiated, validated, and completed.

If you (or one of your customers) wish to be an originator of Direct Debits you will need to apply to your client director separately.
 
#### Bacs Origination
 
As an **Originator / Service User** of Direct Debits, your responsibility is to construct and submit the payment instruction. This involves identifying the source and destination accounts, specifying the amount of the collection, and assigning a transaction reference. You are also responsible for collecting the payer's authority to debit (called a **Direct Debit Instruction**), notifying the payer when the collection will take place and how much will be collected (a process called **Advance Notice**), and all other communication relating to the Direct Debit.
 
This role centres around initiating payments and defining the transaction details. It requires ensuring that each payment is properly prepared and documented prior to being routed through the Bacs network.
 
#### Bacs Clearing
 
Acting as a **Clearing PSP** means applying the transaction to the correct account once it has been output from Bacs to ClearBank. The task includes identifying the correct destination account and performing necessary validation checks. These checks confirm that the account exists, is open, and can accept the payment.
 
In the case of Direct Debits, additional steps are required in:
- Confirming the availability of funds
- Verifying the authority to debit the account

Once all conditions are met, the transaction is recorded in your ledger to complete the process.

This role focuses on validating and applying payments, ensuring that each transaction complies with operational and regulatory requirements before finalisation
