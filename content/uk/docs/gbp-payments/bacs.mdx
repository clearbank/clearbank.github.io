...
This page is for Bacs Payment Service Providers (PSPs). If you are a Service User, refer to [Service Users](../gbp-payments/bacs-service-users).

As a PSP, you receive incoming Direct Debit Instructions (DDIs) from Service Users. DDIs can be used to credit money to, or debit money from, one of your end customer's accounts.

Once a DDI is in place, payments follow the Bacs three‑day cycle:
- Day 1: On the **input day**, the Service User sends Bacs a file of payments.
- Day 2: On the **processing day**, Bacs sends the PSP a series of payments to be processed. 
- Day 3: On the **entry day**, payments are applied to accounts. Bacs does not operate on weekends or bank holidays.

![Image showing 3-day Bacs payment cycle](/assets/images/bacs/BacsOverview-Days.svg "Bacs payment cycle")

Inbound Bacs payments can be either credit or debit payments. This can result in money moving either to or from the end-customer's account.
When a payment is successfully applied to an account, this message flow occurs:

![Image showing a successful Bacs payment](/assets/images/bacs/BacsOverview-PaymentSuccess.svg "Successful Bacs payment")

### Incoming payments

To monitor incoming payments, you can subscribe to the following webhooks:
- [Bacs Direct Debit Inbound Payment Created](#bacs-direct-debit-inbound-payment-created-webhook)
- [Bacs Direct Credit Inbound Payment Created](#bacs-direct-credit-inbound-payment-created-webhook)

You will receive early notification of a payment scheduled to occur on the entry day (day 3). You can use this information to notify the account owner about upcoming funds entering or leaving their account. You can also use the Bacs transaction ID to associate the webhooks that describe each payment.

Note that credit payments can be recalled by the originator up to 4pm on the processing day (day 2). If this happens, you will be notified by a [Bacs Direct Credit Recalled webhook](#bacs-direct-credit-recalled-webhook), and the credit will not be applied to the account.

Whether a payment is successfully applied to the end-customer's account is determined like this:

![Image showing a Inbound Bacs decision tree](/assets/images/bacs/BacsOverview-DecisionTree.svg "Inbound Bacs decision tree")

### Payments that cannot be applied

If the payment cannot be applied to the account, ClearBank will automatically raise an ARUCS (Automated Return of Unapplied Credits Service) item  or an ARUDD (Automated Return of Unpaid Direct Debit) item. You will be notified with a
- [Bacs Direct Credit Return Created](#bacs-direct-credit-return-created-webhook) or 
- [Bacs Direct Debit Return Created](#bacs-direct-debit-return-created-webhook)

If a Direct Debit is paid from a virtual account, it is your responsibility to ensure that the account has sufficient funds. If it does not, use the [returns endpoints]() below.
...
## Direct Credit payment webhooks
...
You can choose to return a payment that has been applied to an account. Returns should be initiated straight away, but can be initiated until 15:30 (UTC) the working day after settlement.
...
![Image showing a Inbound Bacs decision tree](/assets/images/bacs/BacsOverview-ReturnsDC.svg "Message flow: Return a Direct Credit")
...
![Image showing a Inbound Bacs decision tree](/assets/images/bacs/BacsOverview-ReturnsDD.svg "Message flow: Return a Direct Debit")
