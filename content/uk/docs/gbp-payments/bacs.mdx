...
[Bankers' Automated Clearing Services (Bacs)](/uk/docs/api/glossary#bacs) is a payment system used to process transactions between accounts, which operates on a three working-day cycle. A file of payments is sent to Bacs on the Input day (day 1). ClearBank receives details of payments to be made on the Processing day (day 2). Payments are finally applied to accounts on the entry day (day 3). 

<Callout colour="blue">Bacs does <strong>not</strong> operate on weekends or bank holidays.</Callout>

![Image showing 3-day Bacs payment cycle](/assets/images/diagrams/uk/bacs/BacsOverview-Days.svg "Bacs payment cycle")

Inbound Bacs payments can either be credit or debit payments, so the successful payment scenario can result in money moving either to or from the end-customer's account.
When a payment is successfully applied to an account, this message flow takes place:
...
If the payment cannot be applied to the account, ClearBank will automatically raise an[ ARUCS (Automated Return of Unapplied Credits Service)](/uk/docs/api/glossary#arucs) item  or an [ARUDD (Automated Return of Unpaid Direct Debit)](/uk/docs/api/glossary#arudd) item. You will be notified with a [Bacs Direct Credit Return Created](#bacs-direct-credit-return-created-webhook) or [Bacs Direct Debit Return Created](#bacs-direct-debit-return-created-webhook) webhook. Note that if a Direct Debit is paid from a virtual account, it is your responsibility to ensure that the account has sufficient funds. If it does not, use the following returns endpoints.

## Returning a Bacs payment

You can choose to return a payment that has been applied. Returns should be initiated straight away, but can be initiated until 15:30 (UTC) the working day after [settlement](/uk/docs/api/glossary#settlement).

To return a [Direct Credit](/uk/docs/api/glossary#bacs-direct-credit), use the following endpoints and specify the reason for return using a [Direct Credit reason code](#arucs-direct-credit-reason-codes). ClearBank will notify you with a [Bacs Direct Credit Return Created webhook](#bacs-direct-credit-return-created-webhook) on the day the return is created (Day 3 or 4).  The funds will be transferred to your Bacs suspense account on the same day and returned on Day 5 or 6. The message flow is shown here:
...
Use the following endpoint to return payments from a real account. To return payments from a virtual account, use: [/v1/Accounts/{accountId}/Virtual/{virtualAccountId}/Transactions/Returns](#return-bacs-payment-virtual-account)
...
<tbody>
<tr>
<td>0</td><td>Refer to payer</td><td>A payer’s bank is not in a position to pay the Direct Debit; (for some reason other than the exception).
