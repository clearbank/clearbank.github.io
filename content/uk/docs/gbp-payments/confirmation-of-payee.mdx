...
[Confirmation of Payee (CoP)](/uk/docs/api/glossary#confirmation-of-payee) gives you greater assurance that you are sending payments to the intended beneficiary. It not only helps in preventing accidental, misdirected payments but also reduces the risk associated with [Authorised Push Payment (APP) fraud](/uk/docs/api/glossary#app-scam). CoP is a scheme agnostic, peer-to-peer messaging service between accredited participants and sits completely outside of the payment journey. A CoP check can be made prior to setting up a payee or making a payment.
...
If you have chosen to use the ClearBank CoP service, you must PATCH all of your accounts using the following endpoints. New accounts must also be patched to access CoP because account creation and account patching are two separate actions.

<Callout colour="blue">
To opt out specific accounts, first PATCH those accounts, then opt them out using the following PUT endpoints.
</Callout>
...
<Callout colour="blue">
• <b>Duplicate Opt Out requests</b> for an account or virtual account will be handled idempotently. Any duplicate request will result in a 200 (Success) response with the message, "Account already opted out. New reason recorded." <br/><br/>
• <b>Duplicate Opt In requests</b> will be handled idempotently and returned with a 204 (No content) response.**
</Callout>
...
- [POST /open-banking/outbound/v1/name-verification](#check-payee-account-name)
...
- [POST /open-banking/outbound/v1/srd/validate](#check-whether-a-sort-code-account-number-requires-srd)
