...
Real accounts are held physically by ClearBank, reconciling their balances.  
Virtual accounts are registered off the back of real accounts. They allow you to keep several different customers' funds in a single real account.

**Real accounts are physically held with ClearBank.**  
The balances of real accounts is tracked by ClearBank, and can be checked on the ClearBank Portal or via our **[GET /v3/Accounts/\{accountId\}](/uk/docs/gbp-accounts/manage-accounts#get-information-for-an-account-real)** endpoint.  
...
**Virtual accounts are held on your platform, but their funds are kept in a real account held by ClearBank.**  
To register a virtual account with ClearBank, you must link it to one of your real General - also known as Pooled - accounts. You can then use the virtual account's Iban to send and receive payments using funds from the associated real account.  
You can link a virtual account to a ClearBank real account using the **[POST /v2/Accounts/\{accountId\}/Virtual](/uk/docs/gbp-accounts/manage-accounts#create-accounts-virtual)** endpoint.  
...
The `Retail` / `Wholesale` endings will depend on your institution's arrangement with ClearBank.
...
Safeguarded accounts, being a type of segregated account, should not be used to hold any of your institution's own funds.
...
**Bacs Suspense**  
A special account used to settle Bacs payments which cannot be applied to the intended account. You do not need to deposit funds into it, and you cannot initiate payments from it.
