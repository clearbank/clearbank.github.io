...
You _can_ create additional accounts of other types - these are covered later.
...
Real accounts are physically held with ClearBank and can be used to send and receive payments.  
The balance of a real account is held by ClearBank, and can be checked both via the Institution Portal and the ClearBank API.

Real GBP accounts can be created using the **[POST /v3/Accounts endpoint](/uk/docs/gbp-accounts/manage-accounts#create-an-account-real)**.

The **UsageType** field specifies an account's type.  
The account type you choose should depend on your agreement with ClearBank, your FCA permissions, and the purpose of the account.

<Callout colour="blue">
An account's usage type is immutable. Take care to set the correct usage type when making a new account.
</Callout>

The **AccountName** field specifies an account's name and has character restrictions. For accepted characters, refer to the endpoint's documentation.

Account names are automatically prefixed with an abbreviated account-type label, depending on the [account type](#account-types) for example:  
If an account were created with the following parameters:  
  * **AccountName**: 'account one'  
  * **UsageType**: 'Segregated Designated'  

When inspected using the [**GET /v3/Accounts/{accountId}**](/uk/docs/gbp-accounts/manage-accounts#get-information-for-an-account-real) endpoint, the name would be returned as: `Des Seg Wsale account one`
...
Segregation separates your operational funds from your customer's funds. For certain types of institution, it is a regulatory requirement that customer funds are segregated from your institution's operational funds. Accounts created for this purpose are called segregated accounts.
...
The `Retail` / `Wholesale` endings will depend on your institution's settings. (TODO)
sortcode blah
...
**Designated Client**
Also known as **Client Money Designated** accounts - each hold funds for just one of your customers.

### Segregated accounts

Segregation accounts exist to hold customer funds separate from your operational funds. They should therefore not be used to hold any of your institution's own funds.  
You can use segregated accounts if you're not subject to Safeguarding or Client Money rules but you wish to keep your customer's funds separate from your operational funds.
...
Your **Bacs Suspense Account** is a special account used to settle Bacs payments which cannot be applied to the intended account. You do not need to deposit funds into it, and you cannot initiate payments from it.
...
You do not need to create Client Suspense accounts unless instructed to do so. You may require an additional suspense account which is not scheme specific if you adopt certain ClearBank products such as ISAs.