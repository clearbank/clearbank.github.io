...
Before you can integrate with [the ClearBank API](/uk/docs/api/overview), you need to onboard with us as a customer and must have a valid authentication profile.
...
- A [certificate signing request](/uk/docs/api/technical-reference#certificate-signing-request) (CSR) in [PKCS](/uk/docs/api/technical-reference#public-key-cryptography-standards) #10 format*
- A token
...
We use a [digital signature](/uk/docs/api/technical-reference#digital-signature) to ensure that the integrity of a request body (sent to us) is maintained. The whole HTTP request body (in its original format) is used to create a digital signature, as it is specific to the body of each request.
...
5. Send the Base64 encoded string representation of the digital signature in the HTTP header [**DigitalSignature**](/uk/docs/api/technical-reference#digital-signature) for all requests that have a request body.
...
We utilise HTTP-request headers for the ClearBank API calls which include `Authorization`, `DigitalSignature` and `X-Request-Id` headers. In all our responses we will send an `X-Correlation-Id` in the header, together with an [HTTP status code](#http-status-codes).
...
We use [webhooks](/uk/docs/api/technical-reference#webhook) to deliver transaction confirmation notifications and account-specific reconciliations to your system as soon as such events occur on your account. Refer to [Set up and test webhook listener URLs](#set-up-and-test-webhook-listener-urls) for more information.
...
```
### Rate limiting
We use [rate limiting](/uk/docs/api/technical-reference#rate-limit) to protect our systems from excessive load and to ensure a consistent level of availability and performance to all our clients.
...
You must be able to handle duplicate webhooks [idempotently](/uk/docs/api/technical-reference#idempotency). This means you should always respond in the appropriate format even if you have detected a duplicated or retried webhook. If you don't, we won't be able to verify the webhook was successfully received and will therefore retry delivery.
