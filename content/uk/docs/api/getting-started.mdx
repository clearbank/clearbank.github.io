...
**Note:** The `x-retry-reason` header will only be present in the case of a retry, it will not be present for the original webhook attempt. You should therefore not rely on this header to detect duplicates. Instead, you should check for duplicates based on the payload of the webhook.
