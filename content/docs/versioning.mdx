...
We work in an agile environment and iteratively release latest versions of individual endpoints and webhooks.
Our iterative release process ensures that breaking changes which are released will not impact existing endpoints and webhooks (which you have already integrated with).
We define a breaking change as a new material change to an endpoint or webhook, including but not limited to new functionality, a new field or a change to the name of an existing field.

### New versioning policy - effective from 17 February 2023

The following text will replace the existing policy (above) from 17th February 2023.
...
All our API endpoints are versioned through the URI path; for example /v2/Accounts.
