...
Webhook responses must be sent in less than 5 seconds. Any response provided after 5 seconds will be considered a failed webhooks. For this reason we recommend that verification/acceptance should be seperate from the actions you would otherwise take having recived the webhook. Verification/acceptance must be done synchronously but processing should be done asynchronously to ensure you can respond quickly.ClearBank® will retry failed webhooks every 15 minutes for 24 hours unless a successful response is returned by you.
